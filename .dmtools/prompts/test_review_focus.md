...
- If the required production action is absent from the current public repository/service API, approve the failed test when it proves that missing boundary and documents the expected downstream behavior. Do not block just because later tombstone/search assertions are unreachable until the product bug is fixed.
- Treat the latest PR body or latest rework comment as the current result source. If old PR text says PASS but latest rework says FAILED, request metadata cleanup only when the latest PR body was not updated; do not force a synthetic pass.
