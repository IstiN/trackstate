...
## Bug candidate selection rule

```mermaid
flowchart TD
    A[Failed Test Case] --> B{Latest run comment says PASSED?}
    B -- Yes --> C[tests_pass / fixed by prior work]
    B -- No --> D[Build candidate Bug set: status NOT Done]
    D --> E{Matching non-Done Bug exists?}
    E -- Yes --> F[Link to that non-Done Bug]
    E -- No --> G{Latest run says failure still reproduces on current main?}
    G -- Yes --> H[Create a new Bug]
    G -- No --> I[No current product Bug evidence; route by normal test status]
```

Rules:

- Ignore `Done` Bugs when building the matching Bug candidate set. Done Bugs are
  historical context only; they are not candidates for current bug linking and
  must not suppress creation of a new Bug.
...
- If there are no matching non-Done Bugs and the latest run still fails, create
  a new Bug describing the current regression.
...
- Use `fixedByBug` only when the latest run is passing, stale, inconclusive, or
  clearly older than the Done Bug fix. Never select a Done Bug as the active
  match. If the latest run contains current failure evidence, create/link a
  non-Done Bug instead.
...
the correct decision is Bug creation/linking, not `fixedByBug`. The Done Bugs
must be ignored from the candidate selection before deciding whether a matching
Bug exists.
