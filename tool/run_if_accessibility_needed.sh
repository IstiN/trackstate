...
ensure_accessibility_runtime() {
  if [[ "$command_to_run" != *"test:a11y"* && "$command_to_run" != *"playwright"* ]]; then
    return
  fi

  if [[ ! -x node_modules/.bin/playwright ]]; then
    echo "[a11y-gate] Playwright CLI not found in node_modules; running npm ci"
    npm ci
  fi

  echo "[a11y-gate] Ensuring Chromium is available for Playwright"
