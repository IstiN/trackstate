...
void main() {
  test(
    'startup guard blocks interactive shell while auth probe is pending after snapshot load timeout',
    () async {
      ...
      // The snapshot loaded (via fallback timeout), but the auth probe is still pending.
      expect(
        viewModel.isStartupGuardBlockingInteractiveShell,
        isTrue,
        reason:
            'The interactive shell must remain blocked while the auth probe is still pending, even after the snapshot fallback loads.',
      );
...
class _SlowSnapshotRepository extends ProviderBackedTrackStateRepository {
  ...
  Future<TrackerSnapshot> loadSnapshot() async {
    // Simulate a slow snapshot load that takes longer than the hosted startup probe timeout.
  ...
  @override
  TrackerSnapshot buildHostedStartupFallbackSnapshot() => TrackerSnapshot(
