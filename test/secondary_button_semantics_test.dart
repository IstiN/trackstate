...
void main() {
  testWidgets(
    'SecondaryButton exposes a single tappable button semantics node',
    (tester) async {
      ...
      void collectButtonNodes(SemanticsNode node) {
        if (node.getSemanticsData().hasFlag(SemanticsFlag.isButton)) { // ignore: deprecated_member_use
      ...
      expect(
        ...
        reason: 'SecondaryButton should expose exactly one button semantics '
            'node, not a nested outer/inner pair.',
      );
      ...
      expect(
        ...
        isTrue,
        reason: 'The exposed button node must have a tap action so browser '
            'semantics clicks reach the handler.',
      );
  ...
  testWidgets(
    'SecondaryButton disabled state does not expose a tap action',
    (tester) async {
      final semantics = tester.ensureSemantics();
      ...
      final data = tester
          ...
          .getSemanticsData();
      expect(data.hasFlag(SemanticsFlag.isButton), isTrue); // ignore: deprecated_member_use
