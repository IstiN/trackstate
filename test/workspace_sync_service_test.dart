...
void main() {
  test('workspace sync service publishes structured domains for detected changes', () async {
            ...
            _copyIssue(
              ...
              comments: [
                ...
                IssueComment(
                  ...
                  storagePath: '${_issueRoot(issue.storagePath)}/comments/0009.md',
                ),
              ],
            )
          else
  ...
  test('workspace sync service applies hosted backoff after failures', () async {
    final repository = _ThrowingWorkspaceSyncRepository();
    final statuses = <WorkspaceSyncStatus>[];
    var now = DateTime.utc(2026, 5, 14, 10, 0);
    final service = WorkspaceSyncService(
      repository: repository,
      loadSnapshot: () async => await const DemoTrackStateRepository().loadSnapshot(),
      onRefresh: (_) {},
      onStatusChanged: statuses.add,
      now: () => now,
    );
    ...
    expect(
      statuses.last.nextRetryAt,
      DateTime.utc(2026, 5, 14, 10, 1),
    );
  ...
  test(
    ...
    () async {
      ...
      expect(
        refreshes.single.result.domains[WorkspaceSyncDomain.comments]?.issueKeys,
