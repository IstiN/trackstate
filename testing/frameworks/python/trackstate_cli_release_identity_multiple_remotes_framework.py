...
class PythonTrackStateCliReleaseIdentityMultipleRemotesFramework(
    PythonTrackStateCliCompiledLocalFramework,
    TrackStateCliReleaseIdentityMultipleRemotesProbe,
):
    ...
    def observe_release_identity_multiple_remotes(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="trackstate-ts-542-bin-") as bin_dir:
            executable_path = Path(bin_dir) / "trackstate"
            self._compile_executable(executable_path)
            with tempfile.TemporaryDirectory(prefix="trackstate-ts-542-repo-") as temp_dir:
                ...
                return TrackStateCliReleaseIdentityMultipleRemotesValidationResult(
    ...
    def _observe_command(
        ...
        *,
        ...
        sandbox_home = repository_path / ".ts542-home"
        ...
        observation = TrackStateCliCommandObservation(
            ...
            fallback_reason=(
                ...
                "and stripped ambient GitHub credentials so TS-542 exercises the local "
                "release-backed download flow from a repository whose non-GitHub origin "
                "must be bypassed in favor of the GitHub upstream remote."
            ),
...
TS-542 local github-releases multi-remote fixture.
""",
        )
        self._write_file(
        ...
        self._git(repository_path, "config", "--local", "user.name", "TS-542 Tester")
        self._git(
            repository_path,
            "config",
            "--local",
        ...
        self._git(
            ...
            config.upstream_remote_url,
        )
        self._git(repository_path, "add", ".")
        self._git(repository_path, "commit", "-m", "Seed TS-542 fixture")
    ...
    def _capture_repository_state(
        ...
        *,
        ...
        remote_origin_url = (
            ...
            if config.origin_remote_name in remote_names
            else None
        )
        remote_upstream_url = (
            self._git_output(
            ...
            if config.upstream_remote_name in remote_names
            else None
        )
        return TrackStateCliReleaseIdentityMultipleRemotesRepositoryState(
            ...
            expected_output_sha256=(
                self._sha256_file(expected_output) if expected_output.is_file() else None
            ),
            expected_output_preview_lines=(
                self._preview_lines(expected_output, line_count=5)
            ...
            remote_upstream_url=remote_upstream_url or None,
    ...
    def _preview_lines(self, path: Path, *, line_count: int) -> tuple[str, ...]:
