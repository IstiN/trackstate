...
class TrackStateCliReleaseIdentityConflictConfig:
    ...
    def project_json_path(self) -> str:
        return f"{self.project_key}/project.json"
    ...
    def manifest_path(self) -> str:
        return f"{self.issue_root_path}/attachments.json"
    ...
    def from_file(
        ...
        if not isinstance(payload, dict):
            raise ValueError(f"TS-503 config must deserialize to a mapping: {path}")

        runtime_inputs = payload.get("runtime_inputs") or {}
        if not isinstance(runtime_inputs, dict):
            raise ValueError(
                f"TS-503 config runtime_inputs must deserialize to a mapping: {path}",
            )
        ...
        return cls(
            ...
            local_attachment_name=cls._require_string(
                runtime_inputs,
                "local_attachment_name",
                path,
            ),
            local_attachment_content=cls._require_string(
                runtime_inputs,
    ...
    def _optional_string(payload: dict[str, Any], key: str) -> str | None:
        ...
        if not isinstance(value, str):
            raise ValueError(f"TS-503 config runtime_inputs.{key} must be a string.")
    ...
    def _require_string(payload: dict[str, Any], key: str, path: Path) -> str:
        ...
        if not isinstance(value, str) or not value:
            raise ValueError(
                f"TS-503 config runtime_inputs.{key} must be a string in {path}.",
            )
        return value
    ...
    def _require_int(payload: dict[str, Any], key: str, path: Path) -> int:
        ...
        if not isinstance(value, int):
            raise ValueError(
                f"TS-503 config runtime_inputs.{key} must be an integer in {path}.",
            )
        return value
    ...
    def _require_string_list(
        ...
        if not isinstance(value, list) or not value:
            raise ValueError(
                f"TS-503 config runtime_inputs.{key} must be a non-empty list in {path}.",
            )
        items: list[str] = []
        for index, item in enumerate(value):
            if not isinstance(item, str) or not item:
                raise ValueError(
                    "TS-503 config runtime_inputs."
