...
class TrackStateCliReleaseDownloadIdentityMissingRemoteConfig:
    ticket_command: str
    ...
    def from_file(
        ...
        if not isinstance(payload, dict):
            raise ValueError(
                "TS-543 config must deserialize to a mapping: "
                f"{path}"
            )
        ...
        if not isinstance(runtime_inputs, dict):
            raise ValueError(
                "TS-543 config runtime_inputs must deserialize to a mapping: "
                f"{path}"
            )
    ...
    def _require_string(payload: dict[str, Any], key: str, path: Path) -> str:
        ...
        if not isinstance(value, str) or not value:
            raise ValueError(
                f"TS-543 config runtime_inputs.{key} must be a string in {path}."
            )
        return value
    ...
    def _require_int(payload: dict[str, Any], key: str, path: Path) -> int:
        ...
        if isinstance(value, int):
            return value
        raise ValueError(
            f"TS-543 config runtime_inputs.{key} must be an integer in {path}."
        )
    ...
    def _require_string_list(
        ...
        if not isinstance(value, list) or not value:
            raise ValueError(
                f"TS-543 config runtime_inputs.{key} must be a non-empty list in {path}."
            )
        items: list[str] = []
        for index, item in enumerate(value):
            if not isinstance(item, str) or not item:
                raise ValueError(
                    "TS-543 config runtime_inputs."
    ...
    def _require_lower_string_list(
        ...
        return tuple(
            item.lower()
            for item in TrackStateCliReleaseDownloadIdentityMissingRemoteConfig._require_string_list(
                payload,
                key,
                path,
            )
    ...
    def _optional_lower_string_list(
        ...
        if not isinstance(value, list):
            raise ValueError(
                f"TS-543 config runtime_inputs.{key} must be a list in {path}."
            )
        items: list[str] = []
        for index, item in enumerate(value):
            if not isinstance(item, str) or not item:
