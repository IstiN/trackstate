...
class TrackStateCliReleaseDownloadAuthFailureConfig:
    ...
    @classmethod
    def from_file(cls, path: Path) -> "TrackStateCliReleaseDownloadAuthFailureConfig":
        payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
        if not isinstance(payload, dict):
            raise ValueError(
                "TS-522 config must deserialize to a mapping: "
                f"{path}"
            )
        ...
        if not isinstance(runtime_inputs, dict):
            raise ValueError(
                "TS-522 config runtime_inputs must deserialize to a mapping: "
                f"{path}"
            )
    ...
    def _require_int(payload: dict[str, Any], key: str, path: Path) -> int:
        ...
        if isinstance(value, int):
            return value
        raise ValueError(f"TS-522 config runtime_inputs.{key} must be an integer in {path}.")
    ...
    def _require_string_list(
        ...
        if not isinstance(value, list) or not value:
            raise ValueError(
                f"TS-522 config runtime_inputs.{key} must be a non-empty list in {path}."
            )
        items = []
        for index, item in enumerate(value):
            if not isinstance(item, str) or not item:
                raise ValueError(
                    "TS-522 config runtime_inputs."
    ...
    def _optional_string_list(
        ...
        if not isinstance(value, list):
            raise ValueError(
                f"TS-522 config runtime_inputs.{key} must be a list in {path}."
            )
        items = []
        for index, item in enumerate(value):
            if not isinstance(item, str) or not item:
