...
class ReleaseOnMergeConfig:
    ...
    @classmethod
    def from_file(cls, path: Path) -> "ReleaseOnMergeConfig":
        payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
        if not isinstance(payload, dict):
            raise ValueError(f"TS-230 config must deserialize to a mapping: {path}")

        runtime_inputs = payload.get("runtime_inputs") or {}
        if not isinstance(runtime_inputs, dict):
            raise ValueError(
                f"TS-230 config runtime_inputs must deserialize to a mapping: {path}"
            )
        ...
        instance = cls(
            ...
            release_timeout_seconds=cls._require_positive_int(
                runtime_inputs,
    ...
    def _require_string(
        ...
        if not isinstance(value, str) or not value.strip():
            raise ValueError(f"TS-230 config is missing runtime_inputs.{key} in {path}.")
        return value.strip()
    ...
    def _require_positive_int(
        ...
        *,
        ...
        if not isinstance(value, int) or value <= 0:
            raise ValueError(
                f"TS-230 config runtime_inputs.{key} must be a positive integer in {path}."
            )
        return value
