...
TICKET_KEY = "TS-543"
TICKET_SUMMARY = (
    "Release-backed local download without Git remotes reports an explicit "
...
TEST_FILE_PATH = "testing/tests/TS-543/test_ts_543.py"
RUN_COMMAND = "python testing/tests/TS-543/test_ts_543.py"


class Ts543ReleaseDownloadIdentityMissingRemoteScenario:
    def __init__(self) -> None:
        self.repository_root = REPO_ROOT
        self.config_path = self.repository_root / "testing/tests/TS-543/config.yaml"
    ...
    def _assert_exact_command(
        ...
        if observation.requested_command != self.config.requested_command:
            failures.append(
                "Precondition failed: TS-543 did not execute the supported local "
        ...
        if observation.compiled_binary_path is None:
            failures.append(
                "Precondition failed: TS-543 must run a repository-local compiled binary "
    ...
    def _assert_initial_fixture(
        ...
        if not initial_state.attachments_metadata_exists:
            failures.append(
                ...
                "with release-backed metadata before running TS-543.\n"
                f"Observed state:\n{_describe_state(initial_state)}"
            )
        if self.config.attachment_relative_path not in initial_state.metadata_attachment_ids:
            failures.append(
                ...
                "manual.pdf entry required for TS-543.\n"
                f"Observed state:\n{_describe_state(initial_state)}"
            )
        if initial_state.expected_output_exists:
            failures.append(
                "Precondition failed: the seeded repository already contained the expected "
                "download output file before TS-543 ran.\n"
                f"Observed state:\n{_describe_state(initial_state)}"
            )
        if initial_state.remote_names:
            failures.append(
                "Precondition failed: TS-543 requires a local Git repository with no "
    ...
    def _validate_runtime(
        ...
        if not missing_identity_fragments and not has_provider_capability_context:
            ...
            _record_human_verification(
                ...
                check=(
                    ...
                    "with repository-identity guidance tied to the missing local Git remote "
                    "configuration."
                ),
        ...
        if has_provider_capability_context:
            ...
            failures.append(
                ...
                "repository identity guidance required by TS-543.\n"
...
def main() -> None:
    ...
    scenario = Ts543ReleaseDownloadIdentityMissingRemoteScenario()
...
def _write_pass_outputs(result: dict[str, object]) -> None:
    ...
    jira_lines = [
        ...
        "",
        "h4. What was tested",
        f"* Executed {_jira_inline(_as_text(result.get('ticket_command')))} from a disposable local TrackState repository whose {_jira_inline('attachments.json')} points {_jira_inline('manual.pdf')} at GitHub release tag {_jira_inline(_as_text(result.get('attachment_release_tag')))} and whose Git repository has no remotes configured.",
        ...
        "",
        "h4. Result",
        "* Step 1 passed: the CLI failed immediately with explicit repository-identity guidance for the missing local Git remote configuration.",
    ...
    markdown_lines = [
        ...
        "",
        "## What was automated",
        f"- Executed `{_as_text(result.get('ticket_command'))}` from a disposable local TrackState repository whose `attachments.json` points `manual.pdf` at GitHub release tag `{_as_text(result.get('attachment_release_tag'))}` and whose Git repository has no remotes configured.",
        "- Removed ambient GitHub credentials from the runtime environment and inspected the caller-visible CLI error output.",
...
def _write_failure_outputs(result: dict[str, object]) -> None:
    ...
    jira_lines = [
        ...
        "",
        "h4. What was tested",
        f"* Executed {_jira_inline(_as_text(result.get('ticket_command')))} from a disposable local TrackState repository whose {_jira_inline('attachments.json')} points {_jira_inline('manual.pdf')} at {_jira_inline('storageBackend = github-releases')} and whose Git repository has no remotes configured.",
    ...
    markdown_lines = [
        ...
        "",
        "## What was automated",
        f"- Executed `{_as_text(result.get('ticket_command'))}` from a disposable local TrackState repository whose `attachments.json` points `manual.pdf` at `storageBackend = github-releases` and whose Git repository has no remotes configured.",
        "- Inspected the caller-visible CLI output, the local output path, and the manifest metadata after the command.",
        "",
        "## Result",
        f"- ❌ Step 1 failed: {step_one_summary}.",
    ...
    bug_lines = [
        ...
        f"- Release tag referenced by metadata: `{_as_text(result.get('attachment_release_tag'))}`",
        "- Auth setup: `GH_TOKEN`, `GITHUB_TOKEN`, and `TRACKSTATE_TOKEN` were removed from the process environment before execution.",
        "",
        "## Steps to reproduce",
        (
            "1. ✅ Create a local TrackState repository whose `attachments.json` contains "
            "a `manual.pdf` entry with `storageBackend = github-releases`, and ensure the "
            "local Git repository has no remotes configured. Observed: the fixture "
            "repository opened normally, contained `TS-123`, had no Git remotes, and "
            "the manifest contained `TS/TS-123/attachments/manual.pdf`."
        ),
        (
            "2. ❌ Execute CLI command from the ticket scenario: "
            "`trackstate attachment download --issue TS-123 --file manual.pdf --target local`. "
            "Observed via the current supported equivalent "
            f"`{_as_text(result.get('ticket_command'))}`: exit code "
            f"`{_as_text(result.get('exit_code'))}` and visible output `{visible_error}`."
        ),
        (
            f"3. ✅ Inspect the command output, local filesystem path `{expected_output_path}`, "
            "and the local manifest. Observed: the file was not created, stdout/stderr are "
            "captured below, and the manifest attachment ids remained unchanged."
        ),
        "",
        "## Expected result",
        "- The command should fail with a specific error that says the GitHub repository identity could not be resolved from local remotes because no remote is configured.",
        "- It must not return the generic `This repository provider does not support GitHub Releases attachment downloads` error.",
