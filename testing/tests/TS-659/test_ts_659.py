...
class TrackStateCliSelfLinkGuardTest(unittest.TestCase):
    maxDiff = None

    def setUp(self) -> None:
        self.repository_root = Path(__file__).resolve().parents[3]
        self.config = TrackStateCliSelfLinkGuardConfig.from_defaults()
        self.validator = TrackStateCliSelfLinkGuardValidator(
            probe=create_trackstate_cli_self_link_guard_probe(self.repository_root)
        )

    def test_link_to_self_returns_validation_error_without_persisting_metadata(
        self,
    ) -> None:
        ...
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Precondition failed: TS-659 must create Issue A in the disposable "
                "Local Git repository before attempting the self-referencing link."
            ),
        )
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Step 1 failed: TS-659 must execute the exact self-link command from "
                "the ticket against the disposable Local Git repository."
            ),
        )
        ...
        self.assertEqual(
            ...
            "local-git",
            "Step 2 failed: the self-link CLI response did not identify the Local "
        ...
        self.assertIsInstance(
            ...
            "the self-referencing link attempt.\n"
            f"Observed payload: {self_link_payload}",
        )
        ...
        self.assertEqual(
            ...
            "Expected result failed: the CLI did not classify the self-link attempt "
            "as a validation mutation error.\n"
            f"Observed error payload: {error_payload}",
        )
        self.assertEqual(
            error_payload.get("category"),
            self.config.expected_error_category,
            "Expected result failed: the CLI did not report the self-link attempt "
            "under the validation error category.\n"
            f"Observed error payload: {error_payload}",
        )
        self.assertEqual(
            ...
            "exit code for a self-referencing link.\n"
            f"Observed error payload: {error_payload}",
        )
        ...
        self.assertIsInstance(
            ...
            "descriptive message for the self-referencing link attempt.\n"
            f"Observed error payload: {error_payload}",
        )
        ...
        for fragment in self.config.expected_error_message_fragments:
            self.assertIn(
                ...
                "that the issue cannot be linked to itself.\n"
        ...
        self.assertEqual(
            ...
            self.config.issue_a_key,
            "Expected result failed: the CLI error details did not preserve the "
            "source issue key from the attempted self-link command.\n"
            f"Observed error details: {error_details}",
        )
        ...
        self.assertEqual(
            ...
            (),
            "Expected result failed: the self-link mutation still produced a "
            "`links.json` file even though the CLI should reject the relationship.\n"
        ...
        self.assertIsNone(
            ...
            "metadata after the self-link attempt should have been rejected.\n"
        ...
        self.assertIsNone(
            ...
            "Expected result failed: the source issue still has parsed link metadata "
            "after the self-link attempt should have been rejected.\n"
            f"Unexpected path: {observation.links_json_relative_path}\n"
        ...
        for fragment in (
            ...
            self.assertIn(
                ...
                "attempting to create a self-referencing relationship.\n"
    ...
    def _assert_command_was_executed_exactly(
        ...
        *,
        ...
        self.assertIsNotNone(
            ...
            f"{precondition_message}\n"
            "TS-659 must execute a repository-local compiled binary so the seeded "
        ...
        self.assertEqual(
            ...
            observation.compiled_binary_path,
            f"{precondition_message}\n"
            "TS-659 did not run the compiled repository-local CLI binary.\n"
            f"Executed command: {observation.executed_command_text}\n"
    ...
    def _assert_failed_envelope(
        ...
        *,
        ...
        self.assertEqual(
            ...
            self.config.expected_error_exit_code,
            f"{failure_prefix}: executing `{observation.requested_command_text}` did "
            "not return the expected validation exit code for a self-referencing "
            "link.\n"
            f"Repository path: {observation.repository_path}\n"
            f"Executed command: {observation.executed_command_text}\n"
            f"Observed exit code: {observation.result.exit_code}\n"
        ...
        self.assertFalse(
            ...
            "reported success for a self-referencing link.\n"
            f"Observed payload: {payload}",
        )
        return payload
