...
SUCCESS_SCREENSHOT_PATH = OUTPUTS_DIR / "ts1290_success.png"
FAILURE_SCREENSHOT_PATH = OUTPUTS_DIR / "ts1290_failure.png"
REPLACE_DIALOG_TIMEOUT_MS = 120_000
...
def main() -> None:
    ...
    if not token:
        raise RuntimeError(
            "TS-1290 requires GH_TOKEN or GITHUB_TOKEN to open the hosted live app.",
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="ts1290-", dir=OUTPUTS_DIR) as temp_dir:
            ...
            with create_live_tracker_app_with_stored_token(
                ...
                try:
                    ...
                    try:
                        ...
                        failure_observation = (
                            "after clicking `Upload attachment`, the live hosted UI did not "
                            "render the explicit replacement confirmation dialog within "
                            f"{REPLACE_DIALOG_TIMEOUT_MS // 1000} seconds.\n"
                            "Expected the browser flow to pause on a visible confirmation "
                            "dialog before any replacement completed.\n"
                            "Observed repository text before confirmation:\n"
                            f"{repo_text_without_confirmation}\n"
                            "Observed body text after clicking Upload attachment:\n"
                            f"{body_text_after_upload_click}"
                        )
                        _record_step(
                            result,
                            step=3,
                            status="failed",
                            action=(
                                "Observe the UI response after the repository preflight "
                                "check."
                            ),
                            observed=failure_observation,
                        )
                        _record_human_verification(
                            result,
                            check=(
                                "Verified as a user that pressing `Upload attachment` never "
                                "surfaced a visible replacement confirmation dialog."
                            ),
                            observed=body_text_after_upload_click,
                        )
...
def _assert_preconditions(issue_fixture: LiveHostedIssueFixture) -> None:
    if issue_fixture.key != "DEMO-2":
        raise AssertionError(
            "Precondition failed: TS-1290 expected the seeded DEMO-2 fixture.\n"
...
def _jira_comment(result: dict[str, object], *, passed: bool) -> str:
    ...
    if not passed:
        lines.append(
            ...
            + (
                "product-facing TS-1290 failure after reaching the Attachments flow."
...
def _response_summary(result: dict[str, object], *, passed: bool) -> str:
    ...
    if not passed:
        lines.extend(
            [
                (
                    "- Failure classification: product-facing TS-1290 failure."
                    if _is_product_failure(result)
...
def _bug_description(result: dict[str, object]) -> str:
    lines = [
        ...
        f"- Attachments body text after upload click: `{result.get('attachments_body_text_after_upload_click', '')}`",
