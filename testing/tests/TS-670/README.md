...
Verifies that the low-level `trackstate read ticket --key TS-2` command exposes
the inward side of a symmetric `relates to` link through the canonical top-level
`links` array.
...
```bash
python -m unittest testing.tests.TS-670.test_ts_670
```
