...
class JiraCompatibleCliSearchLastPageTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.config = TrackStateCliJiraSearchConfig(
            ...
            fixture_issues=tuple(
                TrackStateCliJiraSearchFixtureIssue(
                    key=f"TRACK-{issue_number}",
                    summary=f"Issue {issue_number}",
                    assignee=f"user{issue_number}",
                    reporter=f"user{issue_number}",
                )
    ...
    def test_search_command_reports_last_page_for_final_issue(self) -> None:
        ...
        self.assertEqual(
            ...
            "Precondition failed: TS-340 did not execute the supported local-search "
        ...
        self.assertEqual(
            ...
            "Precondition failed: the seeded repository fixture did not return only "
            "the final TRACK issue during the supported control search.\n"
            f"Observed payload: {control_payload}",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "Precondition failed: TS-340 did not execute the exact ticket command.\n"
        ...
        self.assertIsNotNone(
            ...
            "Precondition failed: TS-340 must execute a repository-local compiled "
        ...
        self.assertEqual(
            ...
            observation.compiled_binary_path,
            "Precondition failed: TS-340 did not run the compiled repository-local "
            "CLI binary.\n"
            f"Executed command: {observation.executed_command_text}\n"
        ...
        self.assertEqual(
            ...
            "--startAt 4 --maxResults 1` did not succeed from a valid TrackState "
            "repository.\n"
        ...
        self.assertEqual(
            ...
            list(self.config.expected_issue_keys),
            "Expected result failed: the search response did not include only the "
            "final issue from the seeded TRACK project.\n"
            f"Observed data: {data}",
        )
        self.assertEqual(
            data.get("startAt"),
        ...
        self.assertEqual(
            data.get("total"),
            self.config.expected_total,
            "Expected result failed: `data.total` did not report the full number of "
            "matching issues.\n"
            f"Observed data: {data}",
        )
        self.assertIsInstance(
        ...
        self.assertIs(
            ...
            "Expected result failed: `data.isLastPage` did not report that the final "
            "offset reached the end of the result set.\n"
            f"Observed data: {data}",
        )
        ...
        for fragment in (
            ...
            self.assertIn(
                ...
                "the expected final-page search fields and issue content.\n"
