...
usable and keeping the production-visible `Create issue` flow available with
the core system fields.
...
3. waits for the production-visible startup state instead of asserting immediately
4. verifies startup stays usable without a data-load crash
...
```text
Pass: the missing config directory still leaves the app usable and the Create
issue form still renders Summary, Description, Save, and Cancel so the user can
continue with the core fallback fields.
