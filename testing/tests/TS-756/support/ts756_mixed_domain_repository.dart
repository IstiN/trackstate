...
class Ts756MixedDomainRepository extends ProviderBackedTrackStateRepository {
  Ts756MixedDomainRepository._(this._provider) : super(provider: _provider);

  factory Ts756MixedDomainRepository() =>
      Ts756MixedDomainRepository._(_Ts756MutableProvider());

  static const String issueCKey = 'TRACK-741C';
  static const String issueCSummary = 'Issue C mixed sync allow-list guard';
  static const String initialComment =
      'Issue C original comment is visible before the mixed sync event arrives.';
  static const String updatedComment =
      'Issue C synced comment appears after the valid comments path is processed while the unknown metadata path is ignored.';
  static const String validChangedPath = 'TRACK-741C/comments/0001.md';
  static const String unknownChangedPath = 'TRACK-741C/unknown-metadata';

  final _Ts756MutableProvider _provider;
  final List<Ts756HydrationCall> hydrateCalls = <Ts756HydrationCall>[];
  ...
  Future<void> emitMixedDomainRefresh() => _provider.emitMixedDomainRefresh();
...
class Ts756HydrationCall {
  const Ts756HydrationCall({
...
class _Ts756MutableProvider implements TrackStateProviderAdapter {
  ...
  _Ts756MutableProvider() {
    _rebuildFiles();
  }

  final Map<String, _Ts756IssueRecord> _issues = <String, _Ts756IssueRecord>{
    Ts756MixedDomainRepository.issueCKey: _Ts756IssueRecord(
      key: Ts756MixedDomainRepository.issueCKey,
      summary: Ts756MixedDomainRepository.issueCSummary,
      issueTypeId: 'story',
      statusId: 'todo',
      priorityId: 'high',
      updated: '2026-05-15T11:10:00Z',
      description:
          'Issue C stays selected while hosted workspace sync processes mixed valid and unknown domain paths.',
      comments: const <String>[Ts756MixedDomainRepository.initialComment],
  ...
  String get repositoryRevision => 'ts756-revision-$_revision';

  String issueDescription(String key) => _issue(key).description;

  String issueCommentBody(String key) => _issue(key).comments.join('\n');

  Future<void> emitMixedDomainRefresh() async {
    final issue = _issue(Ts756MixedDomainRepository.issueCKey);
    _issues[issue.key] = issue.copyWith(
      comments: const <String>[Ts756MixedDomainRepository.updatedComment],
  ...
  Future<RepositoryUser> authenticate(RepositoryConnection connection) async =>
      const RepositoryUser(login: 'ts756-user', displayName: 'TS-756 User');
  ...
  Future<RepositoryAttachment> readAttachment(
    ...
    throw const TrackStateProviderException(
      'TS-756 does not require attachment downloads.',
    );
  }
  ...
  Future<RepositoryAttachmentWriteResult> writeAttachment(
    ...
    throw const TrackStateProviderException(
      'TS-756 does not require attachment uploads.',
    );
  }
  ...
  Future<RepositoryTextFile> readTextFile(
    ...
    if (content == null) {
      throw TrackStateProviderException('Missing TS-756 fixture for $path.');
    }
  ...
  Future<RepositoryCommitResult> createCommit(
    ...
    throw const TrackStateProviderException(
      'TS-756 should not create commits.',
    );
  }
  ...
  RepositorySyncState _syncState() => RepositorySyncState(
    ...
    sessionRevision: 'ts756-session',
    connectionState: ProviderConnectionState.connected,
    permission: _permission,
  );

  String get _currentRevision => 'ts756-r$_revision';

  _Ts756IssueRecord _issue(String key) {
    final issue = _issues[key];
    if (issue == null) {
      throw StateError('Missing TS-756 issue state for $key.');
    }
    return issue;
  }
  ...
  String _issueMarkdown(_Ts756IssueRecord issue) {
...
class _Ts756IssueRecord {
  const _Ts756IssueRecord({
    required this.key,
  ...
  _Ts756IssueRecord copyWith({
    String? updated,
    String? description,
    List<String>? comments,
  }) {
    return _Ts756IssueRecord(
      key: key,
      summary: summary,
