...
class Ts559DraftReleaseCreationScenario(
    TrackStateCliReleaseAssetFilenameSanitizationScenario,
):
        ...
        if not fixture_failures:
            record_step(
                ...
                action=(
                    ...
                    "and the exact `setup.log` upload file."
                ),
                observed=(
    ...
    def _validate_manifest(
        ...
        if manifest is None or not manifest.matches_expected:
            failures.append(
                ...
                "expected release-backed entry for `setup.log`.\n"
    ...
    def _validate_release(
        ...
        record_step(
            ...
            step=4,
            status="passed",
            action="Verify the remote repository state via `gh release view ts559-assets-TS-999`.",
            observed=(
        ...
        record_human_verification(
            ...
            check=(
                ...
                "titled `Attachments for TS-999` containing the uploaded `setup.log` asset."
            ),
