...
issue while it still has active child issues, preventing the issue hierarchy
from being orphaned.
...
3. verifying the returned typed failure is `validation` with the exact
   user-facing message
4. confirming the blocked delete leaves issue files, tombstone state,
   repository metadata, search visibility, and hierarchy relationships
   unchanged
...
```bash
/opt/hostedtoolcache/flutter/stable-3.35.3-x64/flutter/bin/flutter test testing/tests/TS-623/test_ts_623.dart -r expanded
```
