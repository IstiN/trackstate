...
SUCCESS_SCREENSHOT_PATH = OUTPUTS_DIR / "ts532_success.png"
FAILURE_SCREENSHOT_PATH = OUTPUTS_DIR / "ts532_failure.png"
...
def main() -> None:
    ...
    if not token:
        raise RuntimeError(
            "TS-532 requires GH_TOKEN or GITHUB_TOKEN to open the hosted live app.",
        )
            ...
            try:
                ...
                if runtime.kind != "ready":
                    raise AssertionError(
                        ...
                        "shell before the repository-path Open settings scenario began.\n"
                ...
                if selected_settings_tab_before != STATUSES_TAB_LABEL:
                    raise AssertionError(
                        ...
                        "default to the Statuses tab before verifying the `Open settings` "
                        "recovery action.\n"
                ...
                if download_count != 1:
                    raise AssertionError(
                        "Step 1 failed: the repository-path Attachments tab did not keep "
                        f"exactly one visible download row for `{attachment_name}`.\n"
                        f"Observed download control count: {download_count}\n"
                        f"Observed body text:\n{attachments_body}",
                    )
                if open_settings_count != 1:
                    raise AssertionError(
                        "Step 1 failed: the repository-path restriction notice did not "
                        "expose exactly one visible `Open settings` action before activation.\n"
                ...
                _record_step(
                    ...
                    step=1,
                    status="passed",
                    action="Navigate to an issue and select the 'Attachments' tab.",
                    observed=(
                ...
                _record_human_verification(
                    ...
                    check=(
                        ...
                        "warning title, message, inline Open settings action, and the "
                        "existing attachment row below the notice."
                    ),
                ...
                result["settings_after_click"] = _navigation_state_dict(
                    navigation_state,
                )
                ...
                if settings_observation.selected_tab_label != ATTACHMENTS_TAB_LABEL:
                    raise AssertionError(
                ...
                if not settings_observation.attachment_storage_visible:
                    raise AssertionError(
                        ...
                        "show the visible `Attachment storage mode` section.\n"
                        f"Observed body text:\n{settings_observation.body_text}",
                    )
                ...
                _record_step(
                    ...
                    status="passed",
                    action=(
                        "Click the 'Open settings' button inside the restriction notice."
                    ),
                    observed=(
                ...
                _record_human_verification(
                    result,
                    check=(
                        "Verified as a user that activating Open settings landed on the "
                        "Project Settings screen with the Attachments tab active, showed "
                        "the Attachment storage mode control, and no longer showed the "
                        "Statuses-only Add status action."
                    ),
...
def _assert_preconditions(issue_fixture: LiveHostedIssueFixture) -> None:
    if issue_fixture.key != "DEMO-2":
        raise AssertionError(
            "Precondition failed: TS-532 expected the seeded DEMO-2 issue fixture.\n"
            f"Observed issue key: {issue_fixture.key}",
        )
    if not issue_fixture.attachment_paths:
        raise AssertionError(
            "Precondition failed: TS-532 requires a live issue with visible attachments.\n"
...
def _jira_comment(result: dict[str, object], *, passed: bool) -> str:
    ...
    lines = [
        ...
        (
            ...
            "the live issue *Attachments* tab, and verified the exact repository-path "
            "restriction title/message plus the visible {{Open settings}} action."
        ),
        (
        ...
        "*Observed result*",
        (
            "* Matched the expected result: the hosted app navigated from the repository-path "
            "restriction notice to *Project Settings > Attachments*, showing the "
            "{{Attachment storage mode}} configuration."
            if passed
...
def _pr_body(result: dict[str, object], *, passed: bool) -> str:
    ...
    lines = [
        ...
        "### Observed result",
        (
            "- Matched the expected result: the hosted app navigated from the "
            "repository-path restriction notice to `Project Settings > Attachments`, "
            "showing the `Attachment storage mode` configuration."
            if passed
            else "- Did not match the expected result."
        ),
        (
...
def _response_summary(result: dict[str, object], *, passed: bool) -> str:
    ...
    lines = [
        ...
        "",
        (
            "Ran the live hosted repository-path Attachments recovery-navigation scenario "
            "and checked the visible notice, the inline Open settings action, and the "
...
def _bug_description(result: dict[str, object]) -> str:
    return "\n".join(
        [
            "# TS-532 - Open settings from repository-path notice does not land on Project Settings Attachments configuration",
            "",
            "## Steps to reproduce",
            "1. Navigate to an issue and select the 'Attachments' tab.",
            f"   - {'✅' if _step_status(result, 1) == 'passed' else '❌'} {_step_observation(result, 1)}",
            "2. Click the 'Open settings' button inside the restriction notice.",
            f"   - {'✅' if _step_status(result, 2) == 'passed' else '❌'} {_step_observation(result, 2)}",
            "",
            "## Actual vs Expected",
            (
                "- Expected: activating `Open settings` from the repository-path "
                "Attachments warning should navigate to the visible `Project Settings` "
                "screen, keep the `Attachments` sub-tab active, and show the "
                f"`{EXPECTED_ATTACHMENT_SETTINGS_LABEL}` configuration section."
            ),
            (
                "- Actual: "
                + str(
                    result.get("error")
                    or "the hosted repository-path Open settings flow did not reach the Attachments configuration."
                )
            ),
            "",
