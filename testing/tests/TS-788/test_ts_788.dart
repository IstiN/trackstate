...
import 'support/ts788_partial_allow_list_repository.dart';

const String _ticketKey = 'TS-788';
const String _ticketSummary =
    'Sync event with domain path partially matching allow-list — reload bypassed';
const String _testFilePath = 'testing/tests/TS-788/test_ts_788.dart';
const String _runCommand =
    'mkdir -p outputs && flutter test testing/tests/TS-788/test_ts_788.dart --reporter expanded';
const List<String> _requestSteps = <String>[
  "Simulate a background sync event with a domain path that includes a valid keyword but is malformed (for example 'TRACK-741C/comments_and_more' or 'comments/TRACK-741C').",
  'Monitor the sync service for refresh dispatch activity.',
...
void main() {
  ...
  testWidgets(
    'TS-788 filters malformed hosted paths that only partially match the comments allow-list',
    (tester) async {
      ...
      final repository = Ts788PartialAllowListRepository();
      ...
      try {
        ...
        result['issue_key'] = Ts788PartialAllowListRepository.issueCKey;
        result['malformed_changed_paths'] =
            Ts788PartialAllowListRepository.malformedChangedPaths;
        result['expected_visible_comment'] =
            Ts788PartialAllowListRepository.initialComment;
        result['unexpected_visible_comment'] =
            Ts788PartialAllowListRepository.hiddenUpdatedComment;

        await screen.openSection('Board');
        await screen.openIssue(
          Ts788PartialAllowListRepository.issueCKey,
          Ts788PartialAllowListRepository.issueCSummary,
        );
        ...
        if (!commentsTabOpened ||
          ...
          throw AssertionError(
            'Precondition failed: the production app did not reach Issue-C with the visible Comments tab loaded before the malformed partial allow-list sync paths were simulated.\n'
        ...
        await repository.emitMalformedAllowListRefresh();
        ...
        final stepOneObserved =
            ...
            'malformed_changed_paths=${_formatPathList(Ts788PartialAllowListRepository.malformedChangedPaths)}';
        _recordStep(
          result,
          step: 1,
        ...
        if (syncCheckDelta < 1) {
          failures.add(
            'Step 1 failed: the app-resume background sync did not process the malformed partial allow-list changed paths.\n'
            'Observed: $stepOneObserved',
          );
        }
        ...
        final stepTwoObserved =
            ...
            'expected_visible_comment="${Ts788PartialAllowListRepository.initialComment}"; '
            'unexpected_visible_comment="${Ts788PartialAllowListRepository.hiddenUpdatedComment}"';
        _recordStep(
          result,
          step: 2,
          status:
        ...
        if (!refreshDispatchBlocked ||
            ...
            !issueDetailStillVisible) {
          failures.add(
            'Step 2 failed: the malformed partial allow-list changed paths still dispatched a visible issue refresh instead of being filtered out.\n'
            'Observed: $stepTwoObserved\n'
        ...
        if (loadSnapshotDelta != 0) {
          failures.add(
            "Step 3 failed: the malformed partial allow-list changed paths incremented the global load_snapshot_delta counter instead of leaving it unchanged.\n"
            'Observed: $stepThreeObserved',
          );
        }
        ...
        _recordHumanVerification(
          result,
          check:
              'Kept the real Issue-C Comments tab open after the malformed sync event and checked the exact comment text where a user would read it.',
          observed:
              'initial_comment_still_visible=$initialCommentStillVisible; hidden_updated_comment_visible=$hiddenUpdatedCommentVisible; expected_visible_comment="${Ts788PartialAllowListRepository.initialComment}"; unexpected_visible_comment="${Ts788PartialAllowListRepository.hiddenUpdatedComment}"; visible_texts=${_formatSnapshot(visibleTextsAfterSync)}',
        );
        _recordHumanVerification(
          result,
          check:
              'Confirmed the selected issue detail surface stayed on screen without visibly replacing the original comment with the hidden synced content.',
          observed:
...
String _jiraComment(Map<String, Object?> result, {required bool passed}) {
  ...
  final lines = <String>[
    ...
    '',
    'h4. What was tested',
    '* Published a hosted background sync event whose changed paths only partially matched the comments allow-list ({code}TRACK-741C/comments_and_more{code} and {code}comments/TRACK-741C{code}).',
    ...
    'h4. Result',
    passed
        ? '* Matched the expected result: the malformed partial allow-list paths were filtered out, no refresh dispatches occurred, load_snapshot_delta stayed unchanged, and the original Issue-C comment remained visible.'
...
String _prBody(Map<String, Object?> result, {required bool passed}) {
  ...
  final lines = <String>[
    ...
    '',
    '## What was automated',
    '- Published a hosted background sync event whose changed paths only partially matched the comments allow-list (`TRACK-741C/comments_and_more` and `comments/TRACK-741C`).',
    '- Kept the production Issue-C Comments tab open while the workspace sync listener processed the event.',
...
String _responseSummary(Map<String, Object?> result, {required bool passed}) {
  final buffer = StringBuffer()
    ...
    ..writeln(
      passed
          ? 'Passed: malformed hosted changed paths that only partially matched the comments allow-list were filtered out without a snapshot reload, and the visible Issue-C comment stayed unchanged.'
          : 'Failed: malformed hosted changed paths that only partially matched the comments allow-list still triggered refresh behavior that should have been filtered out.',
    )
    ..writeln()
...
String _bugDescription(Map<String, Object?> result) {
  final lines = <String>[
    ...
    '',
    '## Summary',
    'Malformed hosted changed paths that only partially match the comments allow-list still fall back to production-visible refresh behavior instead of being filtered out.',
    '',
    '## Steps to Reproduce',
    ..._bugStepLines(result),
    '',
    '## Actual vs Expected',
    '- **Expected:** malformed partial allow-list hosted paths are ignored, no refresh dispatches occur, and `load_snapshot_delta` stays unchanged while the visible Issue-C comment remains the original text.',
    '- **Actual:** ${_actualResultLine(result)}',
    '',
    '## Missing/Broken Production Capability',
    '- The production workspace sync domain filter does not safely reject malformed changed paths that include a valid keyword without matching the required allow-list structure. Instead, it still allows reload and/or hydration behavior that mutates the visible issue surface.',
    '',
    ...
    '- Malformed changed paths used: `${_formatDynamicPathList(result['malformed_changed_paths'])}`',
    '',
    '## Relevant Logs',
    '```text',
...
String _formatHydrationCalls(List<Ts788HydrationCall> calls) {
  if (calls.isEmpty) {
    return '<none>';
  }
  return calls
...
String _formatPathList(List<String> paths) {
  if (paths.isEmpty) {
    return '<none>';
  }
  return paths.join(' | ');
...
String _formatDynamicPathList(Object? value) {
  final paths = (value as List<Object?>?)
      ?.map((item) => item.toString())
      .toList(growable: false);
  if (paths == null) {
    return '<missing>';
  }
  return _formatPathList(paths);
}
