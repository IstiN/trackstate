...
Validates the staged-change dirty-save behavior for `DEMO/DEMO-1/main.md`.

The automation covers the ticket in two layers against the same staged local
issue:
1. create a temporary local Git runtime fixture
2. edit and stage `DEMO/DEMO-1/main.md` outside TrackState
...
```bash
flutter test testing/tests/TS-92/test_save_issue_with_staged_changes_test.dart --reporter expanded
```
...
```text
Once TS-78 is fixed, both tests pass:
- the provider-backed staged-write failure becomes actionable
