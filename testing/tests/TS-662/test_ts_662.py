...
class TrackStateCliDistinctIssueLinkPersistenceTest(unittest.TestCase):
    maxDiff = None

    EXPECTED_LINKS_JSON_PAYLOAD = [
        {
            "type": "relates-to",
            "target": "TS-2",
            "direction": "outward",
    ...
    def test_distinct_issue_link_persists_relationship_metadata(self) -> None:
        ...
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Precondition failed: TS-662 must create Issue A in the disposable "
                "Local Git repository before attempting the distinct-issue link."
            ),
        )
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Precondition failed: TS-662 must create Issue B in the disposable "
                "Local Git repository before attempting the distinct-issue link."
            ),
        )
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Step 1 failed: TS-662 must execute the exact `trackstate ticket "
                'link --key TS-1 --target-key TS-2 --type "relates to"` command '
        ...
        self.assertEqual(
            ...
            "ticket-link",
            "Step 2 failed: the link CLI response did not report the expected "
        ...
        self.assertEqual(
            ...
            "Expected result failed: the visible CLI response did not preserve the "
            "canonical outward relationship metadata for the distinct issue link.\n"
            f"Expected link payload: {self.config.expected_canonical_link_payload}\n"
            f"Observed link payload: {observed_link_payload}\n"
            f"Observed stdout:\n{observation.inverse_link_observation.result.stdout}",
        )

        source_links_payload = observation.source_links_json_payload
        self.assertIsInstance(
            source_links_payload,
            list,
            "Step 3 failed: inspecting `TS/TS-1/links.json` did not yield a JSON "
            "array at the expected source issue location.\n"
            f"Expected path: {observation.source_links_json_relative_path}\n"
            f"Observed target path: {observation.target_links_json_relative_path}\n"
            f"Discovered links.json files: {observation.discovered_links_json_files}\n"
            f"Observed files:\n{self._format_links_json_snapshots(observation)}",
        )
        assert isinstance(source_links_payload, list)
        self.assertEqual(
            source_links_payload,
            self.EXPECTED_LINKS_JSON_PAYLOAD,
            "Expected result failed: `TS/TS-1/links.json` did not persist the "
            "canonical stored outward relationship to TS-2.\n"
            f"Expected path: {observation.source_links_json_relative_path}\n"
            f"Expected payload: {self.EXPECTED_LINKS_JSON_PAYLOAD}\n"
            f"Observed payload: {source_links_payload}\n"
            f"Discovered links.json files: {observation.discovered_links_json_files}\n"
            f"Observed files:\n{self._format_links_json_snapshots(observation)}",
        )
        self.assertEqual(
            observation.discovered_links_json_files,
            (observation.source_links_json_relative_path,),
            "Expected result failed: the repository did not store exactly one "
            "`links.json` file on Issue A after linking it to the distinct target "
            "issue.\n"
            f"Expected path: {observation.source_links_json_relative_path}\n"
        ...
        self.assertIsNone(
            observation.target_links_json_content,
            "Expected result failed: the target issue unexpectedly persisted its own "
            "`links.json` file for a symmetric `relates to` command that should stay "
            "anchored on Issue A.\n"
            f"Unexpected path: {observation.target_links_json_relative_path}\n"
            f"Observed content:\n{observation.target_links_json_content}\n"
            f"Observed files detail:\n{self._format_links_json_snapshots(observation)}",
        )
        ...
        for fragment in (
            ...
            self.assertIn(
                fragment,
                observation.inverse_link_observation.result.stdout,
                "Human-style verification failed: the visible CLI success output did "
                "not show the relationship confirmation a user would read in the "
                "terminal after linking two different issues.\n"
    ...
    def _assert_command_was_executed_exactly(
        ...
        *,
        ...
        self.assertIsNotNone(
            ...
            f"{precondition_message}\n"
            "TS-662 must execute a repository-local compiled binary so the seeded "
        ...
        self.assertEqual(
            ...
            observation.compiled_binary_path,
            f"{precondition_message}\n"
            "TS-662 did not run the compiled repository-local CLI binary.\n"
            f"Executed command: {observation.executed_command_text}\n"
    ...
    @staticmethod
    def _build_config() -> TrackStateCliInverseLinkCanonicalStorageConfig:
        return TrackStateCliInverseLinkCanonicalStorageConfig(
            test_id="TS-662",
            project_key="TS",
            project_name="TS-662 Distinct Issue Link Persistence Project",
            seed_issue_key="TS-0",
            expected_author_email="ts662@example.com",
