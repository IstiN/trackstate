...
class TrackStateCliInverseCloneLinkCanonicalStorageTest(unittest.TestCase):
    maxDiff = None
    ...
    def test_inverse_clone_link_label_is_persisted_in_canonical_direction(self) -> None:
        ...
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Precondition failed: TS-628 must create Issue A in the disposable "
                "Local Git repository before attempting the inverse clone link."
            ),
        )
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Precondition failed: TS-628 must create Issue B in the disposable "
                "Local Git repository before attempting the inverse clone link."
            ),
        )
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Step 1 failed: TS-628 must execute the exact inverse-label link "
        ...
        self.assertEqual(
            ...
            "the canonical stored `clones` relation from Issue B back to Issue A.\n"
            f"Expected canonical path: {observation.target_links_json_relative_path}\n"
        ...
        for fragment in (
            ...
            self.assertIn(
                ...
                "see immediately after running the inverse clone-label command.\n"
    ...
    def _assert_command_was_executed_exactly(
        ...
        *,
        ...
        self.assertIsNotNone(
            ...
            f"{precondition_message}\n"
            "TS-628 must execute a repository-local compiled binary so the seeded "
        ...
        self.assertEqual(
            ...
            observation.compiled_binary_path,
            f"{precondition_message}\n"
            "TS-628 did not run the compiled repository-local CLI binary.\n"
            f"Executed command: {observation.executed_command_text}\n"
    ...
    def _build_config() -> TrackStateCliInverseLinkCanonicalStorageConfig:
        return TrackStateCliInverseLinkCanonicalStorageConfig(
            test_id="TS-628",
            project_key="TS",
            project_name="TS-628 Inverse Clone Link Canonical Storage Project",
            seed_issue_key="TS-0",
            expected_author_email="ts628@example.com",
