...
```bash
mkdir -p outputs && PYTHONPATH=. python3 testing/tests/TS-1007/test_ts_1007.py
```
...
- Opens the live workspace switcher in pristine state and confirms the footer still renders the visible disabled `Save and switch` boundary control.
...
- Keeps the final pass/fail decision scoped to the ticketed single-step `Shift+Tab` assertion that must land on the disabled footer button while the panel remains open.
