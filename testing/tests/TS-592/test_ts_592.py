...
TICKET_KEY = "TS-592"
TICKET_SUMMARY = (
    "Local upload with non-GitHub remote reports an explicit repository identity error"
)
...
TEST_FILE_PATH = "testing/tests/TS-592/test_ts_592.py"
RUN_COMMAND = "python testing/tests/TS-592/test_ts_592.py"


class Ts592NonGitHubRemoteIdentityScenario:
    def __init__(self) -> None:
        self.repository_root = REPO_ROOT
        self.config_path = self.repository_root / "testing/tests/TS-592/config.yaml"
    ...
    def execute(self) -> tuple[dict[str, object], list[str]]:
        ...
        if not fixture_failures:
            _record_step(
                ...
                status="passed",
                action=(
                    "Prepare a local github-releases fixture repository whose configured "
                    "remotes do not point to GitHub."
                ),
                observed=(
    ...
    def _build_result(
        ...
        return {
            ...
            "expected_origin_remote_name": self.config.origin_remote_name,
            "expected_origin_remote_url": self.config.origin_remote_url,
    ...
    def _assert_exact_command(
        ...
        if observation.requested_command != self.config.requested_command:
            failures.append(
                "Precondition failed: TS-592 did not execute the exact ticket command.\n"
        ...
        if observation.compiled_binary_path is None:
            failures.append(
                "Precondition failed: TS-592 must run a repository-local compiled binary "
    ...
    def _assert_initial_fixture(
        ...
        if initial_state.expected_attachment_exists or initial_state.stored_files:
            failures.append(
                ...
                "attachment before TS-592 ran.\n"
                f"Observed state:\n{_describe_state(initial_state)}"
            )
        expected_remote_names = (
            [self.config.origin_remote_name] if self.config.origin_remote_name else []
        )
        if list(initial_state.remote_names) != expected_remote_names:
            failures.append(
                "Precondition failed: TS-592 requires a local Git repository whose only "
                "configured remote is a non-GitHub origin.\n"
                f"Expected remote names: {expected_remote_names}\n"
                f"Observed state:\n{_describe_state(initial_state)}"
            )
    ...
    def _validate_runtime(
        ...
        if observation.result.exit_code == 0:
            failures.append(
                ...
                "repository is configured for github-releases storage with only a "
                "non-GitHub remote configured.\n"
        ...
        if provider_capability_fragments_present:
            result["failure_mode"] = "provider_capability_gate"
            result["product_gap"] = (
                "Local release-backed uploads still fail through the generic provider "
                "capability gate when the repository only has non-GitHub remotes, instead "
                "of surfacing the explicit repository-identity error."
            )
            failures.append(
                "Step 1 failed: the visible CLI error still matched the old generic "
                "provider-capability failure instead of the explicit repository-identity "
                "error for non-GitHub remotes.\n"
                f"Provider-capability fragments present: {provider_capability_fragments_present}\n"
                f"Visible output:\n{visible_error}\n"
                f"{_format_supporting_evidence(payload=payload, stdout=stdout, stderr=stderr)}"
            )
            return failures
        ...
        result["product_gap"] = (
            ...
            "identity cannot be resolved from the local Git configuration when remotes are "
            "configured but none point to GitHub."
        )
        failures.append(
            "Step 1 failed: the visible CLI error did not state that GitHub repository "
            "identity could not be resolved because no GitHub remote is configured.\n"
    ...
    def _validate_repository_state(
        ...
        if list(final_state.remote_names) != expected_remote_names:
            failures.append(
                "Step 2 failed: the scenario mutated the configured remotes even though "
                "the non-GitHub origin precondition should stay intact.\n"
                f"Observed state:\n{_describe_state(final_state)}"
            )
        if final_state.remote_origin_url != self.config.origin_remote_url:
            failures.append(
                "Step 2 failed: the origin remote changed during the failed upload.\n"
                f"Observed state:\n{_describe_state(final_state)}"
            )
        ...
        if not failures:
            ...
            _record_step(
                ...
                observed=(
                    ...
                    f"remote_origin_url={final_state.remote_origin_url}"
                ),
            )
            _record_human_verification(
                result,
                check=(
                    "Verified from a user's perspective that the exact terminal output "
                    "reported the non-GitHub-remote repository-identity problem and no "
...
def main() -> None:
    ...
    scenario = Ts592NonGitHubRemoteIdentityScenario()
...
def _write_pass_outputs(result: dict[str, object]) -> None:
    ...
    jira_lines = [
        ...
        "",
        "h4. Result",
        "* ✅ Step 1 passed: the CLI failed immediately with explicit repository-identity guidance tied to configured remotes that do not point to GitHub.",
        ...
        "* ✅ Human-style verification passed: the real terminal output a user would see clearly explained that no GitHub remote was configured, and no fallback attachment file appeared in the repository.",
        "",
        "h4. Test file",
        "{code}",
        TEST_FILE_PATH,
...
def _write_failure_outputs(result: dict[str, object]) -> None:
    ...
    if failure_mode == "generic_repository_open_error":
        ...
        human_summary = (
            ...
            "attachment file, but the text a user would see still reported the generic "
            "provider-capability problem rather than explaining that no configured remote "
            "pointed to GitHub."
        )
        actual_result_line = (
            "* However, the visible output still matched the old generic provider-"
            f"capability failure ({_jira_inline(visible_error)}) instead of explicit "
            "repository-identity guidance tied to non-GitHub remotes."
        )
    else:
        step_one_summary = (
            "the command failed, but the visible output still did not explain that "
            "GitHub repository identity could not be resolved because no configured "
            "remote points to GitHub"
        )
        human_summary = (
            "Human-style verification observed a terminal error and no local attachment "
            "file, but the visible error text did not match the expected non-GitHub-"
            "remote repository-identity guidance."
        )
        actual_result_line = (
            "* However, the command returned a failure "
    ...
    bug_lines = [
        ...
        "",
        "## Steps to reproduce",
        "1. ✅ Configure the project with `attachmentStorage.mode = github-releases`. Observed: the disposable fixture repository opened normally and contained `TS-475` with no seeded attachments.",
        f"2. ✅ Configure the local Git repository with a non-GitHub `origin` remote. Observed: `git remote` returned `{json.dumps((final_state or {}).get('remote_names', []))}` and `git remote get-url origin` returned `{remote_url}`.",
        f"3. ❌ Execute CLI command: `{_as_text(result.get('ticket_command'))}`. Observed: exit code `{_as_text(result.get('exit_code'))}` with visible output `{visible_error}`.",
        f"4. ❌ Inspect the command output and error code. Observed: {actual_result_line.replace('* ', '', 1).replace('{{', '`').replace('}}', '`')}",
        f"5. ✅ Inspect the repository attachment path. Observed: `{expected_path}` was not created and the repository stayed clean.",
        "",
        "## Expected result",
        ...
        "- No attachment file was written locally, so the failure is isolated to the caller-visible validation outcome.",
        "",
