...
from testing.tests.support.trackstate_cli_release_replacement_upload_failure_scenario import (  # noqa: E402
    TrackStateCliReleaseReplacementUploadFailureScenario,
)

TICKET_KEY = "TS-609"
TICKET_SUMMARY = (
    "Re-upload collision during GitHub asset upload returns explicit 422 "
    "already_exists and preserves the manifest"
)
...
TEST_FILE_PATH = "testing/tests/TS-609/test_ts_609.py"
RUN_COMMAND = "python testing/tests/TS-609/test_ts_609.py"


class Ts609ReleaseReplacementUploadFailureScenario(
    TrackStateCliReleaseReplacementUploadFailureScenario,
):
    def __init__(self) -> None:
        super().__init__(
            repository_root=REPO_ROOT,
            test_directory="TS-609",
...
def main() -> None:
    ...
    scenario = Ts609ReleaseReplacementUploadFailureScenario()
...
def _jira_comment(result: dict[str, object], *, status: str) -> str:
    ...
    lines = [
        ...
        (
            "* Mocked the CLI release lookup so the existing asset was hidden from the "
            "replacement path, then forced {POST /repos/{owner}/{repo}/releases/"
            "{release_id}/assets} to return HTTP 422 with {code=already_exists}."
        ),
        (
            "* Verified the command failed visibly and that the manifest plus live release "
    ...
    if status == "PASSED":
        lines.extend(
            [
                ...
                (
                    "* The observed behavior matched the expected result: the CLI surfaced "
                    "the 422 already_exists collision clearly, and TrackState did not record "
                    "a new manifest revision for the failed upload."
                ),
            ],
        )
    else:
...
def _markdown_summary(result: dict[str, object], *, status: str) -> str:
    ...
    lines = [
        ...
        (
            "- Mocked the release-tag lookup to hide the existing asset from the CLI and "
            "forced `POST /repos/{owner}/{repo}/releases/{release_id}/assets` to return "
            "HTTP 422 with `already_exists`."
        ),
        (
    ...
    if status == "PASSED":
        lines.extend(
            [
                ...
                (
                    "- The observed behavior matched the expected result: the upload "
                    "collision was reported as an explicit 422 `already_exists` failure and "
                    "`attachments.json` stayed unchanged."
                ),
            ],
        )
    else:
...
def _bug_description(result: dict[str, object]) -> str:
    ...
    visible_output = compact_text(as_text(result.get("visible_output")))
    lines = [
        f"# {TICKET_KEY} - Re-upload collision 422 already_exists is not handled correctly",
        "",
        "## Steps to reproduce",
        (
        ...
        (
            "2. ✅ Mock the GitHub release-tag lookup seen by the CLI so the existing "
            "`data.csv` asset is hidden from the replacement flow."
        ),
        (
            "3. ✅ Force `POST /repos/{owner}/{repo}/releases/{release_id}/assets` to "
            "return HTTP 422 with `code=already_exists`."
        ),
        (
            f"4. ❌ Execute the exact CLI command `{as_text(result.get('ticket_command'))}` "
            "and observe the user-visible output."
        ),
        f"   - Actual behavior: {as_text(result.get('error'))}",
        f"   - Visible output: `{visible_output or '<empty>'}`",
        (
            "5. ❌ Inspect the local `attachments.json` entry and live release asset state "
            "for `data.csv`."
        ),
        ...
        "## Expected vs Actual",
        (
            "- **Expected:** the command should fail explicitly with a clear 422 "
            "`already_exists` upload-collision message, and `attachments.json` should "
            "remain unchanged so TrackState does not record a new revision for a failed "
            "asset upload."
        ),
        (
            f"- **Actual:** {as_text(result.get('error')) or 'The observed behavior did not match the expected 422 collision handling.'} "
            f"Visible output: `{visible_output or '<empty>'}`."
        ),
        "",
