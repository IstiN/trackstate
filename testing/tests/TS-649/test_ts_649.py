...
class TrackStateCliSymmetricRelatesToCanonicalPayloadTest(unittest.TestCase):
    maxDiff = None
    ...
    def test_symmetric_relates_to_link_returns_canonical_relationship_metadata(
        self,
    ) -> None:
        ...
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Precondition failed: TS-649 must create Issue A in the disposable "
                "Local Git repository before attempting the symmetric link."
            ),
        )
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Precondition failed: TS-649 must create Issue B in the disposable "
                "Local Git repository before attempting the symmetric link."
            ),
        )
        self._assert_command_was_executed_exactly(
            ...
            precondition_message=(
                "Step 1 failed: TS-649 must execute the exact `relates to` link "
        ...
        self.assertEqual(
            ...
            "ticket-link",
            "Step 2 failed: the symmetric-link CLI response did not report the "
            "expected `ticket-link` command metadata.\n"
        ...
        self.assertEqual(
            ...
            "symmetric `relates to` label with canonical outward metadata.\n"
        ...
        self.assertEqual(
            ...
            self.config.issue_a_key,
            "Expected result failed: the visible CLI response should remain "
            "anchored to Issue A after returning the canonical symmetric link "
            "metadata.\n"
        ...
        self.assertEqual(
            ...
            "show the expected Issue A summary next to the canonical `relates to` "
            "metadata.\n"
            f"Observed issue payload: {issue_payload}\n"
            f"Observed stdout:\n{observation.inverse_link_observation.result.stdout}",
        )
        ...
        for fragment in (
            ...
            f'"summary": "{self.config.issue_a_summary}"',
            ...
            self.assertIn(
                ...
                observation.inverse_link_observation.result.stdout,
                "Human-style verification failed: the visible CLI response did not "
                "show the expected confirmation details a user would see "
                "immediately after running the symmetric link command.\n"
    ...
    def _assert_command_was_executed_exactly(
        ...
        *,
        ...
        self.assertIsNotNone(
            ...
            f"{precondition_message}\n"
            "TS-649 must execute a repository-local compiled binary so the seeded "
        ...
        self.assertEqual(
            ...
            observation.compiled_binary_path,
            f"{precondition_message}\n"
            "TS-649 did not run the compiled repository-local CLI binary.\n"
            f"Executed command: {observation.executed_command_text}\n"
    ...
    def _build_config() -> TrackStateCliInverseLinkCanonicalStorageConfig:
        return TrackStateCliInverseLinkCanonicalStorageConfig(
            test_id="TS-649",
            project_key="TS",
            project_name="TS-649 Symmetric Relates To Link Project",
            seed_issue_key="TS-0",
            expected_author_email="ts649@example.com",
