...
No extra secrets are required. The test uses `testing/tests/TS-39/config.yaml`, the shared hosted-runtime fixture client from `testing/fixtures/hosted_runtime_client_fixture.dart`, and the app's default startup configuration. The Local Git override probe additionally runs `testing/tests/TS-39/support/ts39_runtime_define_override_probe_test.dart` with `--dart-define=TRACKSTATE_RUNTIME=local-git` on IO platforms.
