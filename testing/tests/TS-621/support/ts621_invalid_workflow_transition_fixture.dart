...
class Ts621InvalidWorkflowTransitionFixture {
  Ts621InvalidWorkflowTransitionFixture._(this.directory);

  static const projectKey = 'TRACK';
  static const issueKey = 'TRACK-621';
  ...
  static const issueSummary =
      'Reject illegal To Do to Done workflow transition';
  ...
  static Future<Ts621InvalidWorkflowTransitionFixture> create() async {
    ...
    final fixture = Ts621InvalidWorkflowTransitionFixture._(directory);
  ...
  observePersistedRepositoryState() async {
    return Ts621PersistedRepositoryObservation(
  ...
  Future<void> _seedRepository() async {
    ...
    await _writeFile(
      ...
      '{"key":"$projectKey","name":"TrackState TS-621"}\n',
    );
    await _writeFile(
...
''');
    ...
    await _git(['config', '--local', 'user.name', 'TS-621 Tester']);
    await _git(['config', '--local', 'user.email', 'ts621@example.com']);
    await _git(['add', '.']);
    await _git([
      'commit',
      '-m',
      'Seed TS-621 invalid workflow transition fixture',
    ]);
  }
...
class Ts621PersistedRepositoryObservation {
  const Ts621PersistedRepositoryObservation({
