...
5. requires restore-specific blocked-window overlap proof while the workspace
...
```text
Pass: while the workspace is still blocked during startup, the app exposes
restore-specific overlap proof that the blocked local restore path was reached,
and after release the prepared active local workspace restores as the selected
Local Git row without keeping Hosted setup workspace active or showing Local
Unavailable.

Fail: while still blocked, startup exposes no restore-specific overlap proof, or
after release it still keeps Hosted setup workspace active, leaves the local
row Unavailable, or otherwise does not restore the saved local workspace as
Local Git.
```
