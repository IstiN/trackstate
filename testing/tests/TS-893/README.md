...
5. requires tracked File System Access activity on the saved local workspace
  lineage while it is still blocked before access is restored, records any
  TS-893 runtime failure probe, then waits after the busy-state release for
  the workspace switcher trigger to restore the saved local workspace instead
