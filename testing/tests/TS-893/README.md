...
5. requires restore-specific blocked-window diagnostics while the workspace is
  still blocked, such as tracked File System Access activity on the saved local
  workspace lineage, a TS-893 runtime probe event, the visible restore skip
  banner, or another public pre-release non-restored state
...
evidence specific to the blocked restore attempt, and after the busy-state
...
Fail: the run cannot prove startup entered the blocked restore/retry path before
release, or after release startup still keeps Hosted setup workspace active,
leaves the local row Unavailable, or otherwise does not restore the saved local
