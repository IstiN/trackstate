...
TICKET_KEY = "TS-561"
TICKET_SUMMARY = "Asset container conflict - fail on foreign assets presence"
...
TEST_FILE_PATH = "testing/tests/TS-561/test_ts_561.py"
RUN_COMMAND = "python testing/tests/TS-561/test_ts_561.py"


class Ts561LocalForeignAssetConflictScenario:
    def __init__(self) -> None:
        self.repository_root = REPO_ROOT
        self.config_path = self.repository_root / "testing/tests/TS-561/config.yaml"
    ...
    def execute(self) -> tuple[dict[str, object], list[str]]:
        ...
        if runtime_failures:
            _mark_product_failure(
                result,
                "The local github-releases upload flow did not fail with the expected "
                "foreign-asset conflict for TS-561.",
            )
        else:
            _record_step(
                result,
        ...
        if remote_failures:
            ...
            _record_human_verification(
                ...
                check=(
                    ...
                    "showed only the foreign asset and did not absorb "
                    "`trackstate-file.txt`."
                ),
    ...
    def _assert_exact_command(
        ...
        if observation.requested_command != self.config.requested_command:
            failures.append(
                "Precondition failed: TS-561 did not execute the exact ticket command.\n"
        ...
        if observation.compiled_binary_path is None:
            failures.append(
                "Precondition failed: TS-561 must run a repository-local compiled binary "
    ...
    def _assert_cleanup(self, result: dict[str, object]) -> list[str]:
        ...
        if cleanup.get("status") == "failed":
            return [
                "Cleanup failed after the TS-561 observation completed.\n"
...
def main() -> None:
    ...
    scenario = Ts561LocalForeignAssetConflictScenario()
...
def _response(result: dict[str, object], *, passed: bool) -> str:
    lines = [
        ...
        (
            f"- Verified the user-visible conflict output, local manifest state, and live "
            f"release state for `{_as_text(result.get('issue_key'))}`."
        ),
        "",
        "## Result",
        *_step_lines(result, jira=False),
...
def _bug_description(result: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            f"# {TICKET_KEY} - Local release-backed upload does not fail on foreign assets presence",
            "",
            "h4. Environment",
            ...
            "1. Configure the project with `attachmentStorage.mode = github-releases` and prepare a valid release container for `TS-123` that already contains a foreign asset `external-file.zip` not tracked in `attachments.json`.",
            (
                f"   - {'✅' if _step_status(result, 1) == 'passed' else '❌'} "
                "Observed precondition: "
                f"{_step_observation(result, 1) or '<not reached>'}"
            ),
            "2. Execute `trackstate attachment upload --issue TS-123 --file trackstate-file.txt --target local`.",
            (
                f"   - {'✅' if _step_status(result, 2) == 'passed' else '❌'} "
                f"Observed command result: {_step_observation(result, 2) or visible_output or _as_text(result.get('error'))}"
            ),
            "3. Inspect the command output.",
            (
                f"   - {'✅' if _step_status(result, 2) == 'passed' else '❌'} "
                f"Visible output at failure point:\n{visible_output or '<empty>'}"
            ),
            "4. Verify the local manifest and release assets after the failed attempt.",
            (
                f"   - {'✅' if _step_status(result, 3) == 'passed' else '❌'} "
                f"Local state: {_step_observation(result, 3) or '<not reached>'}"
            ),
            (
                f"   - {'✅' if _step_status(result, 4) == 'passed' else '❌'} "
                f"Release state: {_step_observation(result, 4) or '<not reached>'}"
            ),
            "",
            "h4. Expected Result",
            "* The command fails with a conflict error indicating that the release contains unexpected foreign assets.",
            "* The system does not auto-delete or absorb `external-file.zip`.",
            "* Local `attachments.json` remains unchanged.",
            "",
            "h4. Actual Result",
            f"* {actual_result}",
            "",
