...
class Ts420SectionReadinessRepository implements TrackStateRepository {
  const Ts420SectionReadinessRepository();
  ...
  Future<TrackerSnapshot> loadSnapshot() async {
    ...
    final issues = snapshot.issues.isEmpty
        ? snapshot.issues
        : <TrackStateIssue>[
            snapshot.issues.first.copyWith(
              hasDetailLoaded: false,
              hasCommentsLoaded: false,
              hasAttachmentsLoaded: false,
            ),
            ...snapshot.issues.skip(1),
          ];
