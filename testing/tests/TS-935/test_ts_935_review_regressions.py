...
def _load_ts_935_module():
    module_path = Path(__file__).with_name("test_ts_935.py")
    spec = importlib.util.spec_from_file_location("ts_935_runtime", module_path)
...
class Ts935ReviewRegressionTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.module = _load_ts_935_module()

    def _observation(
        self,
        *,
        accessibility_conclusion: str = "success",
        downstream_conclusion: str = "success",
        downstream_status: str = "completed",
    ) -> GitHubAccessibilityPullRequestGateObservation:
        accessibility_job = GitHubActionsWorkflowJobObservation(
            id=4561,
        ...
        downstream_job = GitHubActionsWorkflowJobObservation(
            id=4562,
            name="Deploy preview",
            status=downstream_status,
            conclusion=downstream_conclusion,
            html_url="https://github.com/IstiN/trackstate/actions/runs/456/job/4562",
            started_at="2026-05-22T09:11:00Z",
            completed_at="2026-05-22T09:11:20Z",
        )
        return GitHubAccessibilityPullRequestGateObservation(
            ...
            pull_request_head_branch="ts935-accessibility-pass-deploy",
            pull_request_head_sha="abc123",
            pull_request_probe_path="lib/ts935_probe_surface.dart",
            probe_render_host_path="lib/main.dart",
            probe_rendered_in_application=True,
            pull_request_file_paths=["lib/main.dart", "lib/ts935_probe_surface.dart"],
            ...
            observed_run_jobs=[accessibility_job, downstream_job],
            observed_job_names=["Accessibility checks", "Deploy preview"],
            observed_step_names=[
                "Build web app for accessibility scan",
                "Run axe-core accessibility checks",
                "Publish preview",
    ...
    def test_step_4_accepts_successful_downstream_deploy(self) -> None:
        result: dict[str, object] = {"steps": [], "human_verification": []}
        failures: list[str] = []
        observation = self._observation()

        self.module._evaluate_downstream_execution(  # type: ignore[attr-defined]
            result,
            observation=observation,
            jobs=list(observation.observed_run_jobs),
            accessibility_job=observation.observed_run_jobs[0],
            downstream_job=observation.observed_run_jobs[1],
            failures=failures,
        )

        self.assertEqual(failures, [])
        self.assertEqual(result["steps"][0]["status"], "passed")
        self.assertIn("executed successfully", result["steps"][0]["observed"])

    def test_step_4_rejects_skipped_downstream_deploy(self) -> None:
        result: dict[str, object] = {"steps": [], "human_verification": []}
        failures: list[str] = []
        observation = self._observation(downstream_conclusion="skipped")
        ...
        self.assertIn("did not execute successfully", failures[0])

    def test_step_4_rejects_neutral_accessibility_result(self) -> None:
        result: dict[str, object] = {"steps": [], "human_verification": []}
        failures: list[str] = []
        observation = self._observation(accessibility_conclusion="neutral")
        observation = GitHubAccessibilityPullRequestGateObservation(
            **{
                **observation.to_dict(),
                "observed_run_jobs": list(observation.observed_run_jobs),
                "target_workflow": observation.target_workflow,
        ...
        self.assertEqual(len(failures), 1)
        self.assertEqual(result["steps"][0]["status"], "failed")
        self.assertIn("did not conclude with `success`", failures[0])
