...
def main() -> None:
    ...
    if not token:
        raise RuntimeError(
            "TS-605 requires GH_TOKEN or GITHUB_TOKEN to open the hosted live app.",
        )
            ...
            try:
                ...
                if runtime.kind != "ready":
                    raise AssertionError(
                        ...
                        "shell before the repository access keyboard-navigation scenario "
                        "started.\n"
                ...
                try:
                    ...
                    _record_step(
                        ...
                        step=2,
                        status="failed",
                        action="Click into or focus the Fine-grained token input field.",
                        observed=str(error),
                    )
                    raise AssertionError(
                        "Step 2 failed: focusing the visible Fine-grained token input "
                        "field did not leave keyboard focus in that field.\n"
                        f"{error}",
                    ) from None
                ...
                _record_step(
                    ...
                    step=3,
                    status="passed",
                    action="Press the Tab key.",
                    observed="Sent Tab from the Fine-grained token field.",
                )
                try:
                    first_tab_focus = settings_page.wait_for_repository_access_focus(
                    ...
                    raise AssertionError(
                        "Step 4 failed: after pressing Tab from the Fine-grained token "
                        "field, focus did not move to the visible Remember on this "
                        "browser checkbox.\n"
                        f"{error}",
                    ) from None
                _append_focus_observation(result, step=4, observation=first_tab_focus)
                _record_step(
                    result,
                    step=4,
                    status="passed",
                ...
                _record_step(
                    ...
                    step=5,
                    status="passed",
                    action="Press the Tab key again.",
                    observed="Sent Tab from the Remember on this browser checkbox.",
                )
                try:
                    second_tab_focus = settings_page.wait_for_repository_access_focus(
                        "Connect token",
                        timeout_ms=5_000,
                    )
                except AssertionError as error:
                    ...
                    raise AssertionError(
                        "Step 6 failed: after pressing Tab again, focus did not move to "
                        "the visible Connect token button.\n"
                        f"{error}",
                    ) from None
                _append_focus_observation(result, step=6, observation=second_tab_focus)
                _record_step(
                    result,
                    step=6,
                    status="passed",
                    action="Observe the focus indicator location.",
                    observed=_focus_summary(second_tab_focus),
                )
                ...
                _record_human_verification(
                    ...
                    check=(
                        ...
                        "Repository access controls and advanced in sequence from the token "
                        "field to Remember on this browser and then to Connect token."
                    ),
...
def _jira_comment(result: dict[str, object], *, passed: bool) -> str:
    ...
    lines = [
        ...
        (
            "* Focused the Fine-grained token field and checked live keyboard Tab "
            "traversal reached the visible checkbox first and the visible Connect token "
            "action second."
        ),
        "",
        "*Observed result*",
        (
            "* Matched the expected result: the first Tab moved focus to {{Remember on "
            "this browser}} and the second Tab moved focus to {{Connect token}} without "
            "skipping either control."
            if passed
...
def _bug_description(result: dict[str, object]) -> str:
    return "\n".join(
        [
            "# TS-605 - Tab navigation from Fine-grained token does not reach Remember and Connect in order",
            "",
            "## Steps to reproduce",
            "1. Navigate to Project Settings → Repository access.",
            f"   - {'✅' if _step_status(result, 1) == 'passed' else '❌'} {_step_observation(result, 1)}",
            "2. Click into or focus the Fine-grained token input field.",
            ...
            "## Actual vs Expected",
            (
                "- Expected: after the Fine-grained token field has focus, the first Tab "
                "press should move focus to `Remember on this browser` and the second Tab "
                "press should move focus to `Connect token`, while all three controls stay "
                "visibly present in the Repository access section."
            ),
            (
                "- Actual: "
                + str(
