...
4. checks the exact title, explanatory copy, and visible **Open settings** CTA
...
Create issue surface, shows the read-only access callout with the exact
explanation, and exposes an Open settings CTA that routes the user to Project
Settings.

Fail: the Create issue trigger stays silently inert, the create surface opens
without the read-only gate, the visible copy is missing/wrong, or the recovery
CTA does not take the user to the settings/authentication surface.
```
