...
4. pre-seeds the issue release container with one `data.csv` asset and writes a
   matching `attachments.json` entry
5. runs the exact ticket command `trackstate attachment upload --issue TS-123 --file data.csv --target local`
6. verifies the live GitHub Release still contains exactly one `data.csv` asset
...
```bash
python testing/tests/TS-558/test_ts_558.py
```
...
- **Pass:** the CLI succeeds, local `attachments.json` updates from the seeded
  ...
  `data.csv` asset whose downloaded bytes match the replacement payload.
- **Fail:** the CLI fails, leaves duplicate `data.csv` assets, keeps the old
