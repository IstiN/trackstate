...
def create_github_accessibility_wrapper_failure_probe(
    repository_root: Path,
    *,
    config_path: Path | None = None,
) -> GitHubAccessibilityPullRequestGateProbe:
    config = GitHubAccessibilityPullRequestGateConfig.from_file(
        config_path or repository_root / "testing/tests/TS-969/config.yaml"
    )
    return GitHubAccessibilityWrapperFailureProbeService(
        config,
        github_api_client=GhCliApiClient(repository_root),
    )


def create_github_accessibility_wrapper_failure_run_log_reader(
    repository_root: Path,
) -> GitHubWorkflowRunLogReader:
