...
def create_provider_unexpected_operation_exception_probe(
    repository_root: Path,
) -> ProviderSessionSyncProbe:
    return ProviderSessionSyncInspector(
        repository_root,
        runtime=PythonDartProbeRuntime(repository_root),
        probe_root=Path("testing/tests/TS-118/dart_probe"),
        entrypoint=Path("bin/provider_unexpected_operation_exception_probe.dart"),
    )
