...
from testing.components.pages.github_actions_page import GitHubActionsPage
...
class GitHubActionsPageContext(AbstractContextManager[GitHubActionsPage]):
    def __init__(
        self,
    ...
    def __enter__(self) -> GitHubActionsPage:
        self._runtime = self._runtime_factory()
        session = self._runtime.__enter__()
        return GitHubActionsPage(session)
...
def create_github_actions_page(
    *,
    runtime_factory: WebAppRuntimeFactory | None = None,
) -> GitHubActionsPageContext:
    return GitHubActionsPageContext(runtime_factory=runtime_factory)
...
def _default_runtime_factory() -> AbstractContextManager[WebAppSession]:
    try:
        ...
        raise AssertionError(
            "Playwright browser verification is required for GitHub Actions UI "
