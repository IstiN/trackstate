...
def create_github_accessibility_semantics_failure_probe(
    repository_root: Path,
    *,
    config_path: Path | None = None,
) -> GitHubAccessibilityPullRequestGateProbe:
    config = GitHubAccessibilityPullRequestGateConfig.from_file(
        config_path or repository_root / "testing/tests/TS-933/config.yaml"
    )
    return GitHubAccessibilitySemanticsFailureProbeService(
        config,
