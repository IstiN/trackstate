...
def create_github_accessibility_network_timeout_probe(
    repository_root: Path,
    *,
    config_path: Path | None = None,
) -> GitHubAccessibilityPullRequestGateProbe:
    config = GitHubAccessibilityPullRequestGateConfig.from_file(
        config_path or repository_root / "testing/tests/TS-953/config.yaml"
    )
    return GitHubAccessibilityNetworkTimeoutProbeService(
        config,
