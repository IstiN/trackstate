...
TICKET_KEY = "TS-533"
TICKET_SUMMARY = (
    "Local upload with insufficient GitHub permissions — explicit failure for "
...
class Ts533ReleasePermissionFailureScenario:
    def __init__(self) -> None:
        self.repository_root = REPO_ROOT
        self.config_path = self.repository_root / "testing/tests/TS-533/config.yaml"
    ...
    def _build_result(
        ...
        return {
            ...
            "token_source_environment_variable": self.config.token_source_environment_variable,
    ...
    def _assert_exact_command(
        ...
        if observation.requested_command != self.config.requested_command:
            failures.append(
                "Precondition failed: TS-533 did not execute the exact ticket command.\n"
        ...
        if observation.compiled_binary_path is None:
            failures.append(
                "Precondition failed: TS-533 must run a repository-local compiled binary "
    ...
    def _validate_runtime(
        ...
        if observation.result.exit_code == 0:
            failures.append(
                ...
                "authenticated token should not be able to manage GitHub Releases in the "
        ...
        if has_release_context and has_permission_context:
            ...
            _record_human_verification(
                ...
                check=(
                    ...
                    "release-backed permission guidance that explained the authenticated "
                    "account could not manage releases or upload assets."
                ),
                observed=visible_error,
            )
            return failures

        if has_release_context and has_generic_release_auth_context:
            result["failure_mode"] = "generic_release_auth_guidance"
            result["product_gap"] = (
                "The exact local upload path still stops at generic release-backed "
                "auth/configuration guidance instead of surfacing an explicit "
                "permission-denied message for the authenticated GitHub account."
            )
            failures.append(
                "Step 1 failed: the command reached release-backed validation, but the "
                "visible output stayed generic instead of explicitly saying the "
                "authenticated account lacks permission to manage releases or upload "
                "assets.\n"
                f"Visible output:\n{visible_error}\n"
        ...
        result["failure_mode"] = "missing_permission_guidance"
        if not has_release_context:
            failures.append(
                "Step 1 failed: the visible CLI error was not an explicit "
                "release-backed permission failure.\n"
        ...
        if not has_permission_context:
            failures.append(
                "Step 1 failed: the visible CLI error did not explain that the "
                "authenticated account lacks permission to manage releases or upload "
                "assets in the resolved GitHub repository.\n"
                f"Visible output:\n{visible_error}\n"
                f"{_format_supporting_evidence(payload=payload, stdout=stdout, stderr=stderr)}"
            )
        return failures
    ...
    def _validate_repository_state(
        ...
        if final_state.expected_attachment_exists:
            failures.append(
                ...
                "path even though release-backed upload permission should have been denied.\n"
...
def main() -> None:
    ...
    scenario = Ts533ReleasePermissionFailureScenario()
...
def _write_pass_outputs(result: dict[str, object]) -> None:
    ...
    markdown_lines = [
        ...
        f"- Pointed the local Git remote at `{_as_text(result.get('remote_origin_url'))}` and mirrored `{token_source}` into `TRACKSTATE_TOKEN` for the upload attempt.",
        "- Inspected the caller-visible CLI error output and the repository attachment path after the command.",
        "",
        "## Result",
        "- Step 1 passed: the CLI failed immediately with explicit release-backed permission guidance.",
...
def _write_failure_outputs(result: dict[str, object]) -> None:
    ...
    if failure_mode == "generic_release_auth_guidance":
        step_one_summary = (
            "the command stopped with generic release-backed auth/configuration "
            "guidance instead of explicitly explaining that the authenticated "
            "account lacks permission to manage releases or upload assets"
        )
        human_summary = (
            "Human-style verification observed a real terminal failure and no local "
            "attachment file, but the visible message stayed generic and did not tell "
            "the user that the authenticated account lacks release-upload permissions."
        )
        actual_result_line = (
        ...
        step_one_summary = (
            "the command failed, but the visible output did not clearly identify the "
            "release-backed permission problem for the authenticated account"
        )
        human_summary = (
    ...
    markdown_lines = [
        ...
        "## What was automated",
        f"- Executed `{_as_text(result.get('ticket_command'))}` from a disposable local TrackState repository configured with `attachmentStorage.mode = github-releases`.",
        f"- Pointed the local Git remote at `{_as_text(result.get('remote_origin_url'))}` and mirrored `{token_source}` into `TRACKSTATE_TOKEN`.",
        "- Inspected the caller-visible CLI output and the repository attachment path after the command.",
        "",
        "## Result",
        f"- ❌ Step 1 failed: {step_one_summary}.",
    ...
    bug_lines = [
        ...
        (
            ...
            f"Observed: {_jira_inline(expected_path)} was not created, stdout showed the error envelope below, "
            "and the repository stayed clean."
        ),
        "",
        "h4. Expected Result",
        "* The command should fail with a user-visible error explaining that the authenticated GitHub account lacks permission to create releases or upload release assets in the resolved repository.",
