...
directory by running the exact terminal command from the ticket:
...
```bash
python3 -m unittest discover -s testing/tests/TS-132 -p 'test_*.py'
```
...
- `TS132_TARGET_PATH` (optional): target passed to the policy command. Defaults
  to `lib/`.
