...
- Verifies the post-filter rendered `Connected` control itself with golden baselines so the ticket-critical status label is proven from actual pixels, not only the widget or semantics tree.
