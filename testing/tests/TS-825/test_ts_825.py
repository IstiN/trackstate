        ...
        with create_live_tracker_app_with_stored_token(config, token=token) as tracker_page:
            ...
            try:
                ...
                try:
                    ...
                    result["focused_start_field"] = _focused_element_payload(focused_field)
                ...
                _record_step(
                    ...
                    observed=(
                        ...
                        f"focused_field={focused_field.accessible_name!r}"
                    ),
                )
                _record_human_verification(
                    result,
                    check=(
                        ...
                        "title, workspace rows, and in-panel Repository field were visible."
                    ),
                    observed=(
                ...
                try:
                    _assert_key_kept_panel_open(
                        ...
                        expected_focus_label=START_FIELD_LABEL,
                ...
                _record_step(
                    ...
                    observed=(
                        ...
                        f"row_count={arrow_down['switcher']['row_count']}; "
                ...
                _record_step(
                    ...
                    observed=(
                        ...
                        f"row_count={shift['switcher']['row_count']}; "
                ...
                _record_step(
                    ...
                    observed=(
                        ...
                        f"row_count={tab['switcher']['row_count']}; "
                ...
                _record_human_verification(
                    result,
                    check=(
                        "With the visible Repository field focused inside the open panel, "
                        "pressed Arrow Down and Shift and watched the panel stay on screen "
                        "without flashing closed."
                    ),
                    observed=(
                        f"arrow_down_focus={arrow_down['active']['accessible_name']!r}; "
...
def _assert_key_kept_panel_open(
    *,
    ...
    failures: list[str] = []
    if int(switcher.get("row_count", 0)) <= 0:
...
def _jira_comment(result: dict[str, object], *, passed: bool) -> str:
    ...
    lines = [
        ...
        (
            f"* Focused the visible {{{START_FIELD_LABEL}}} field inside the open "
            "panel to exercise in-panel keyboard navigation."
        ),
        (
            "* Pressed Arrow Down, Shift, and Tab while the panel was visible and "
            "verified the panel did not dismiss on any of those keys."
        ),
        (
            "* Verified Tab moved focus from Repository to another visible in-panel "
            "control instead of dismissing the panel."
        ),
        "",
        "h4. Result",
        (
...
def _markdown_summary(result: dict[str, object], *, passed: bool) -> str:
    ...
    lines = [
        ...
        "- Opened the desktop workspace switcher from Dashboard.",
        (
            f"- Focused the visible `{START_FIELD_LABEL}` field inside the open panel "
            "to exercise in-panel keyboard navigation."
        ),
        "- Pressed Arrow Down, Shift, and Tab while the panel was visible.",
        (
            "- Verified the panel remained visible throughout those keys and that Tab "
            "moved focus to another visible in-panel control."
        ),
        "",
        "## Result",
        (
...
def _response_summary(result: dict[str, object], *, passed: bool) -> str:
    ...
    lines = [
        ...
        (
            ...
            if not passed
            else "- Outcome: Arrow Down, Shift, and Tab left the desktop workspace switcher open, and Tab moved focus from Repository to another visible in-panel control."
        ),
    ]
    lines.extend(_artifact_lines(result, jira=False))
    if not passed:
...
def _bug_description(result: dict[str, object]) -> str:
    return "\n".join(
        [
            f"# {TICKET_KEY} - Workspace switcher dismisses or loses in-panel focus on non-Escape keys",
            "",
            "## Steps to reproduce",
