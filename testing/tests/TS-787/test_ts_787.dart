...
import 'support/ts787_empty_domain_path_repository.dart';

const String _ticketKey = 'TS-787';
const String _ticketSummary =
    'Sync event with empty domain path is ignored by filtering logic';
const String _testFilePath = 'testing/tests/TS-787/test_ts_787.dart';
const String _runCommand =
    'mkdir -p outputs && flutter test testing/tests/TS-787/test_ts_787.dart --reporter expanded';
const List<String> _requestSteps = <String>[
  'Simulate a background sync event where the domain path is an empty string ("").',
...
void main() {
  ...
  testWidgets(
    'TS-787 filters empty hosted sync paths without a global reload',
    (tester) async {
      ...
      final repository = Ts787EmptyDomainPathRepository();
      ...
      try {
        ...
        if (!commentsTabOpened ||
          ...
          throw AssertionError(
            'Precondition failed: the production app did not reach Issue-C with the visible Comments tab loaded before the empty sync path was simulated.\n'
        ...
        await repository.emitEmptyDomainRefresh();
        ...
        final stepOneObserved =
            ...
            'changed_path=${jsonEncode(Ts787EmptyDomainPathRepository.emptyChangedPath)}';
        _recordStep(
          result,
          step: 1,
        ...
        if (syncCheckDelta < 1) {
          failures.add(
            'Step 1 failed: the app-resume background sync did not process the empty hosted changed path.\n'
            'Observed: $stepOneObserved',
          );
        }
        ...
        final stepTwoObserved =
            ...
            'expected_visible_comment="${Ts787EmptyDomainPathRepository.initialComment}"; '
            'unexpected_visible_comment="${Ts787EmptyDomainPathRepository.hiddenUpdatedComment}"';
        _recordStep(
          result,
          step: 2,
          status:
        ...
        if (!refreshDispatchBlocked ||
            ...
            !issueDetailStillVisible) {
          failures.add(
            'Step 2 failed: the empty sync path still dispatched a visible issue refresh instead of being filtered out.\n'
            'Observed: $stepTwoObserved\n'
        ...
        if (loadSnapshotDelta != 0) {
          failures.add(
            "Step 3 failed: the empty sync path incremented the global load_snapshot_delta counter instead of leaving it unchanged.\n"
            'Observed: $stepThreeObserved',
          );
        }
        ...
        _recordHumanVerification(
          ...
          observed:
              'initial_comment_still_visible=$initialCommentStillVisible; hidden_updated_comment_visible=$hiddenUpdatedCommentVisible; expected_visible_comment="${Ts787EmptyDomainPathRepository.initialComment}"; unexpected_visible_comment="${Ts787EmptyDomainPathRepository.hiddenUpdatedComment}"; visible_texts=${_formatSnapshot(visibleTextsAfterSync)}',
        );
        _recordHumanVerification(
          result,
          check:
              'Confirmed the selected issue detail surface stayed on screen without visibly replacing the original comment after the empty-path sync attempt.',
          observed:
...
String _jiraComment(Map<String, Object?> result, {required bool passed}) {
  ...
  final lines = <String>[
    ...
    '',
    'h4. What was tested',
    '* Published a hosted background sync event whose changed path was the empty string ({code}""{code}).',
    ...
    'h4. Result',
    passed
        ? '* Matched the expected result: the empty path was filtered out, no refresh dispatches occurred, load_snapshot_delta stayed unchanged, and the original Issue-C comment remained visible.'
...
String _prBody(Map<String, Object?> result, {required bool passed}) {
  ...
  final lines = <String>[
    ...
    '',
    '## What was automated',
    '- Published a hosted background sync event whose changed path was the empty string (`""`).',
    '- Kept the production Issue-C Comments tab open while the workspace sync listener processed the event.',
...
String _responseSummary(Map<String, Object?> result, {required bool passed}) {
  final buffer = StringBuffer()
    ...
    ..writeln(
      passed
          ? 'Passed: the empty hosted sync path was filtered out without a snapshot reload, and the visible Issue-C comment stayed unchanged.'
          : 'Failed: the empty hosted sync path still triggered refresh behavior that should have been filtered out.',
    )
    ..writeln()
...
String _bugDescription(Map<String, Object?> result) {
  final lines = <String>[
    ...
    '',
    '## Summary',
    'An empty hosted sync changed path still falls back to a production-visible reload path instead of being filtered out.',
    '',
    '## Steps to Reproduce',
    ..._bugStepLines(result),
    '',
    '## Actual vs Expected',
    '- **Expected:** an empty hosted sync changed path is ignored, no refresh dispatches occur, and `load_snapshot_delta` stays unchanged while the visible Issue-C comment remains the original text.',
    '- **Actual:** ${_actualResultLine(result)}',
    '',
    '## Missing/Broken Production Capability',
    '- The production workspace sync domain filter does not safely ignore empty hosted change paths. Instead, it still allows reload and/or hydration behavior that mutates the visible issue surface.',
    '',
    ...
    '- Changed path used: `${result['changed_path'] ?? '<missing>'}`',
    '',
    '## Relevant Logs',
    '```text',
...
String _formatHydrationCalls(List<Ts787HydrationCall> calls) {
  if (calls.isEmpty) {
    return '<none>';
  }
  return calls
