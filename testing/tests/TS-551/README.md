...
4. runs the exact ticket command against `TS-123` with a real `data.csv`
   payload
...
```bash
python testing/tests/TS-551/test_ts_551.py
```
...
- A live release fixture for tag `ts-attachments-TS-123` whose title is
  intentionally mismatched from issue `TS-123`
