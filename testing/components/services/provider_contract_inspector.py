...
class ProviderContractInspector(ProviderContractProbe):
    ...
    def inspect(self) -> ProviderContractProbeResult:
        ...
        analyze = self._run(
            [str(dart_bin), "--disable-analytics", "analyze", "bin/provider_contract_probe.dart"],
            cwd=self._probe_root,
            check=False,
        )
        ...
        execution = self._run(
            [str(dart_bin), "--disable-analytics", "run", "bin/provider_contract_probe.dart"],
            cwd=self._probe_root,
        )
        return ProviderContractProbeResult(
            succeeded=True,
    ...
    def _load_runtime_manifest(self) -> dict[str, object]:
        manifest_path = self._probe_root / "runtime_manifest.json"
    ...
    def _archive_config(self) -> dict[str, str]:
        ...
        if platform_key not in archives:
            raise AssertionError(
                f"TS-38 does not define a Dart SDK bootstrap archive for platform '{platform_key}'."
            )
    ...
    def _platform_key() -> str:
        ...
        if system == "darwin" and machine == "arm64":
            return "macos-arm64"
        raise AssertionError(
            f"TS-38 Dart bootstrap is not configured for platform '{system}/{machine}'."
        )

    def _cache_root(self) -> Path:
        configured = os.environ.get("TS38_TOOL_CACHE")
        if configured:
    ...
    def _download_file(url: str, destination: Path) -> None:
        request = urllib.request.Request(url, headers={"User-Agent": "trackstate-ts38"})
