...
class ProviderConnectionFailureInspector(ProviderContractProbe):
    def __init__(self, repository_root: Path, runtime: DartProbeRuntime) -> None:
        self._probe_root = repository_root / "testing/tests/TS-80/dart_probe"
        self._runtime = runtime

    def inspect(self) -> ProviderContractProbeResult:
        execution = self._runtime.execute(
            probe_root=self._probe_root,
            entrypoint=Path("bin/provider_connection_failure_probe.dart"),
        )
