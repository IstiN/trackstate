...
class GitHubAccessibilityEngineLogValidationFailureProbeService(
    GitHubAccessibilityPullRequestGateProbeService
):
    ...
    simulation_technique = (
        "Adds a disposable silent logger wrapper in `testing/accessibility/` so the live "
        "accessibility run omits Flutter engine-state and semantics-discovery log tokens "
        "without changing production app code."
    )

    def _create_and_observe_pull_request(self, workflow_id: int) -> dict[str, object]:
        temp_repository_root = Path(tempfile.mkdtemp(prefix="ts943-"))
        ...
        try:
            ...
            helper_source = self._helper_source()
            helper_file.write_text(helper_source, encoding="utf-8")

            spec_file = temp_repository_root / self._config.probe_render_host_path
            patched_spec_source = self._patch_spec_source(
                spec_file.read_text(encoding="utf-8")
            )
            spec_file.write_text(patched_spec_source, encoding="utf-8")
        ...
        if observation is None:
            raise GitHubAccessibilityPullRequestGateError(
                "TS-943 did not produce a disposable pull request observation."
            )
        return observation

    @staticmethod
    def _helper_source() -> str:
        return """function createTs943SilentEngineLogger() {
  return () => {};
}
    ...
    def _patch_spec_source(cls, source: str) -> str:
        if cls.helper_require_statement not in source:
            ...
            if require_block not in source:
                raise GitHubAccessibilityPullRequestGateError(
                    "TS-943 could not find the shared accessibility gate import block in "
        ...
        original_logging_block = (
            "  await captureFlutterStartupDiagnostics(page, {\n"
            "    log: (entry) => console.log(entry),\n"
            "  });"
        )
        patched_logging_block = (
            "  const silentEngineLogger = createTs943SilentEngineLogger();\n"
            "  await captureFlutterStartupDiagnostics(page, {\n"
            "    log: silentEngineLogger,\n"
            "  });"
        )
        if patched_logging_block in source:
            return source
        if original_logging_block not in source:
            raise GitHubAccessibilityPullRequestGateError(
                "TS-943 could not find the live accessibility logging wrapper in "
                "testing/accessibility/accessibility_gate.spec.js."
            )
        return source.replace(original_logging_block, patched_logging_block, 1)
