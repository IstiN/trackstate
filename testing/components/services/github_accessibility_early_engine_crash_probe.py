...
class GitHubAccessibilityEarlyEngineCrashProbeService(
    GitHubAccessibilityPullRequestGateProbeService
):
    ...
    simulation_call = "  await installTs944EarlyEngineCrashSimulation(page);"
    simulation_technique = (
        "Adds a disposable Playwright network hook that aborts the Flutter web "
        "`main.dart.js` bootstrap asset inside the accessibility harness, forcing a "
        "fatal early-engine startup failure after the page shell loads but before any "
        "surface-ready state appears."
    )

    def _create_and_observe_pull_request(self, workflow_id: int) -> dict[str, object]:
        temp_repository_root = Path(tempfile.mkdtemp(prefix="ts944-"))
        ...
        if observation is None:
            raise GitHubAccessibilityPullRequestGateError(
                "TS-944 did not produce a disposable pull request observation."
            )
        return observation

    @classmethod
    def _simulation_helper_source(cls) -> str:
        return f"""async function installTs944EarlyEngineCrashSimulation(page) {{
  await page.route(/main\\.dart\\.js(?:\\?.*)?$/, async (route) => {{
    console.error({cls.crash_marker!r});
    ...
    def _patch_spec_source(cls, source: str) -> str:
        if cls.helper_require_statement not in source:
            ...
            if import_anchor not in source:
                raise GitHubAccessibilityPullRequestGateError(
                    "TS-944 could not find the shared accessibility gate import block in "
        ...
        if cls.simulation_call not in source:
            ...
            if target not in source:
                raise GitHubAccessibilityPullRequestGateError(
                    "TS-944 could not find the startup diagnostics call in "
                    "testing/accessibility/accessibility_gate.spec.js."
                )
            source = source.replace(
                target,
