...
class ActionlintNonWorkflowPullRequestGateError(RuntimeError):
    pass


class ActionlintNonWorkflowPullRequestGateProbeService:
    _CONTRIBUTOR_VISIBLE_PULL_REQUEST_EVENTS = ("pull_request", "pull_request_target")

    def __init__(
        self,
        config: ActionlintNonWorkflowPullRequestGateConfig,
        *,
    ...
    def validate(self) -> ActionlintNonWorkflowPullRequestGateObservation:
        ...
        actionlint_workflow_text = self._read_workflow_text(
            self._config.actionlint_workflow_path,
            default_branch,
        )
        ...
        return ActionlintNonWorkflowPullRequestGateObservation(
            repository=self._config.repository,
            default_branch=default_branch,
            probe_file_path=self._config.probe_file_path,
            probe_file_present_on_default_branch=self._path_exists_on_default_branch(
                self._config.probe_file_path,
                default_branch,
            ),
            actionlint_workflow_name=workflow_path_to_name.get(
                self._config.actionlint_workflow_path
            ),
            actionlint_workflow_path=self._config.actionlint_workflow_path,
            actionlint_workflow_present_on_default_branch=(
                self._config.actionlint_workflow_path in default_branch_workflow_paths
            ),
            actionlint_workflow_declares_pull_request_trigger=(
                self._workflow_declares_event(actionlint_workflow_text, "pull_request")
            ),
            actionlint_workflow_declares_push_trigger=self._workflow_declares_event(
                actionlint_workflow_text,
                "push",
            ),
            actionlint_workflow_uses_expected_paths_filter=(
                self._config.expected_paths_filter in actionlint_workflow_text
            ),
            ...
            pull_request_probe_marker=str(
                pull_request_observation["pull_request_probe_marker"]
            ),
    ...
    def _create_and_observe_pull_request(
        ...
        *,
        ...
        temp_repository_root = Path(tempfile.mkdtemp(prefix="ts262-"))
        ...
        try:
            ...
            probe_marker = self._append_probe_marker(temp_repository_root)
            self._run_command(
            ...
            run_observation = self._observe_branch_runs(
                branch_name=branch_name,
                started_at=started_at,
                workflow_id_to_path=workflow_id_to_path,
                actionlint_workflow_paths=actionlint_workflow_paths,
            )
            surface_observation = self._wait_for_pull_request_surface(pull_request_number)
            ...
            observation = {
                ...
                "pull_request_probe_marker": probe_marker,
                "cleanup_closed_pull_request": False,
                "cleanup_deleted_branch": False,
                **run_observation,
                **surface_observation,
        ...
        if observation is None:
            raise ActionlintNonWorkflowPullRequestGateError(
                "TS-262 did not produce a disposable pull request observation."
            )
        return observation

    def _append_probe_marker(self, temp_repository_root: Path) -> str:
        probe_path = temp_repository_root / self._config.probe_file_path
        if not probe_path.exists():
            raise ActionlintNonWorkflowPullRequestGateError(
                "TS-262 precondition failed: the configured probe file does not exist.\n"
        ...
        rendered_marker = self._config.probe_marker_template.replace("{marker}", marker)
        with probe_path.open("a", encoding="utf-8") as stream:
            stream.write("\n")
            stream.write(rendered_marker)
            stream.write("\n")
        return rendered_marker

    def _observe_branch_runs(
        self,
        *,
        branch_name: str,
        started_at: float,
        ...
        deadline = time.time() + self._config.actionlint_run_timeout_seconds
            ...
            if candidate is not None:
                ...
                if actionlint_run_status == "completed":
                    actionlint_log_excerpt = self._extract_actionlint_log_excerpt(
                        self._read_actionlint_run_log(int(candidate["run_id"]))
                    )
                    break
    ...
    def _wait_for_pull_request_surface(self, pull_request_number: int) -> dict[str, object]:
        ...
        while time.time() < deadline:
            ...
            if (
                ...
                and (
                    actionlint_check is None
                    or self._status_check_is_terminal(actionlint_check)
                )
            ):
                return latest_state
            time.sleep(self._config.poll_interval_seconds)
    ...
    def _read_pull_request_status_surface(
        ...
        if not isinstance(payload, dict):
            raise ActionlintNonWorkflowPullRequestGateError(
                f"TS-262 expected gh pr view to return an object: {payload!r}"
            )
    ...
    def _path_exists_on_default_branch(self, path: str, default_branch: str) -> bool:
        endpoint = (
            f"/repos/{self._config.repository}/contents/{quote(path, safe='/')}"
            f"?ref={quote(default_branch, safe='')}"
        )
        return self._try_read_json_object(endpoint) is not None

    def _read_workflow_text(self, workflow_path: str, default_branch: str) -> str:
        payload = self._read_json_object(
            f"/repos/{self._config.repository}/contents/{quote(workflow_path, safe='/')}"
            f"?ref={quote(default_branch, safe='')}"
        )
    ...
    def _read_check_runs_state(self, head_sha: str) -> str | None:
        ...
        if isinstance(check_runs, list) and check_runs:
            relevant_runs = [
                ...
                and any(
                    self._config.expected_actionlint_marker.lower() in value.lower()
                    for value in (
    ...
    def _extract_actionlint_log_excerpt(self, log_text: str) -> str:
        ...
        for index, line in enumerate(lines):
            ...
            if (
                self._config.expected_actionlint_marker.lower() in lowered_line
                or self._config.probe_file_path.lower() in lowered_line
            ):
                match_index = index
                break
    ...
    def _close_pull_request(self, pull_request_number: int) -> bool:
        ...
        except ActionlintNonWorkflowPullRequestGateError:
            return False
        return True
    ...
    def _workflow_declares_event(self, workflow_text: str, event_name: str) -> bool:
        return re.search(rf"(?m)^\s*{re.escape(event_name)}\s*:", workflow_text) is not None
    ...
    def _read_json_object(
        ...
        *,
        ...
        try:
            ...
            raise ActionlintNonWorkflowPullRequestGateError(str(error)) from error
    ...
    def _try_read_json_object(self, endpoint: str) -> dict[str, Any] | list[Any] | None:
        try:
    ...
    def _run_names(runs: list[dict[str, Any]]) -> list[str]:
        return [
            name
            for name in (
                ActionlintNonWorkflowPullRequestGateProbeService._optional_string(
                    run.get("name")
                )
                for run in runs
            )
    ...
    def _step_names(jobs: list[dict[str, Any]]) -> list[str]:
        ...
        for job in jobs:
            ...
            for step in steps:
                if not isinstance(step, dict):
                    continue
                name = ActionlintNonWorkflowPullRequestGateProbeService._optional_string(
                    step.get("name")
                )
                if name is not None:
                    names.append(name)
