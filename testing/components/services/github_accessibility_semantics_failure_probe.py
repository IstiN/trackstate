...
class GitHubAccessibilitySemanticsFailureProbeService(
    GitHubAccessibilityPullRequestGateProbeService
):
    simulation_selector = "flt-ts933-simulated-no-semantics"
    simulation_require_statement = (
        "const {\n"
        "  installTs933SemanticsFailureSimulation,\n"
        "} = require('./ts933_semantics_failure_simulation');"
    )
    simulation_call = "  await installTs933SemanticsFailureSimulation(page);"
    simulation_technique = (
        "Adds a disposable Playwright init-script that hides `flt-semantics` nodes from "
        "the accessibility gate polling path, simulating a Flutter engine semantics "
        "exposure failure without changing production app code."
    )

    def _create_and_observe_pull_request(self, workflow_id: int) -> dict[str, object]:
        temp_repository_root = Path(tempfile.mkdtemp(prefix="ts933-"))
        ...
        if observation is None:
            raise GitHubAccessibilityPullRequestGateError(
                "TS-933 did not produce a disposable pull request observation."
            )
        return observation

    @classmethod
    def _simulation_helper_source(cls) -> str:
        return f"""async function installTs933SemanticsFailureSimulation(page) {{
...
"""
    ...
    def _extract_log_excerpt(self, run_log_text: str, fallback_text: str) -> str:
        ...
        prioritized_markers = [
            ...
            "flutter engine failed to render semantics nodes",
            "flutter engine",
            "semantics tree",
            "semantics nodes",
