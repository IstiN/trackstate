...
class LiveWorkspaceSwitcherPage:
    ...
    def click_saved_workspace_action_button(
        ...
        *,
        ...
        escaped_action_label = action_label.replace("\\", "\\\\").replace('"', '\\"')
        try:
            bounds = self._session.wait_for_function(
                """
                ({ actionLabel }) => {
                  const normalize = (value) => (value || '').replace(/\\s+/g, ' ').trim();
                  const isVisible = (element) => {
                  ...
                  const button = Array.from(
                    document.querySelectorAll('flt-semantics[role="button"],button,[role="button"],[aria-label]'),
                  )
                    .filter(isVisible)
                    .find((candidate) => labelFor(candidate) === actionLabel);
                  if (!button) {
                    return null;
                  }
            ...
            if not isinstance(bounds, dict):
                raise WebAppTimeoutError(
                    f'Timed out clicking selector "[aria-label=\\"{escaped_action_label}\\"]".',
                )
            self._session.mouse_click(
                float(bounds.get("left", 0.0)) + (float(bounds.get("width", 0.0)) / 2.0),
                float(bounds.get("top", 0.0)) + (float(bounds.get("height", 0.0)) / 2.0),
            )
        except WebAppTimeoutError as error:
            raise AssertionError(
