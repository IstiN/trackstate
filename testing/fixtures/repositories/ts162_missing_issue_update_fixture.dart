...
class Ts162MissingIssueUpdateFixture {
  Ts162MissingIssueUpdateFixture._({required this.directory});

  final Directory directory;

  static const missingIssueKey = 'MISSING-888';
  ...
  static const updatedDescription =
      'Updated description from the TS-162 repository consumer.';

  static Future<Ts162MissingIssueUpdateFixture> create() async {
    final directory = await Directory.systemTemp.createTemp(
      'trackstate-ts-162-',
    );
    final fixture = Ts162MissingIssueUpdateFixture._(directory: directory);
  ...
  Future<Ts162MissingIssueUpdateObservation> observeBeforeUpdateState() async {
    final repository = LocalTrackStateRepository(
      repositoryPath: directory.path,
    );
    final snapshot = await repository.loadSnapshot();
    return Ts162MissingIssueUpdateObservation(
      snapshot: snapshot,
  ...
  Future<Ts162MissingIssueUpdateObservation>
  updateMissingIssueViaRepositoryService() async {
    final repository = LocalTrackStateRepository(
      repositoryPath: directory.path,
    );
    ...
    try {
      await repository.updateIssueDescription(
        _missingIssue(),
        updatedDescription,
      );
    ...
    if (error == null) {
      throw StateError(
        'Updating $missingIssueKey should fail with a repository not-found error.',
      );
    }
  ...
  TrackStateIssue _missingIssue() {
    return const TrackStateIssue(
      ...
      priorityId: 'medium',
      summary: 'Missing issue update target',
      description:
          'Synthetic missing issue used to verify update error mapping.',
      assignee: '',
      reporter: '',
      labels: [],
      components: [],
  ...
  Future<void> _seedRepository() async {
    ...
    await _writeFile(
      'TRACK/project.json',
      '{"key":"TRACK","name":"Track Update Demo"}\n',
    );
    await _writeFile(
...
This active issue must remain unchanged after the missing update attempt.
''');
    ...
    await _git(['add', '.']);
    await _git(['commit', '-m', 'Seed active issues for TS-162']);
  }
...
class Ts162MissingIssueUpdateObservation {
  const Ts162MissingIssueUpdateObservation({
